<template>
  <div>
    <div style="padding-left: 10px;">
      ...
      <Select v-model="vSelect" style="width:200px; margin-left: 20px;" v-if="vSelectList.length">
    ...
    <div class="chart_layer" style="height: 300px; position: relative; margin: 10px 0;">
      <div ref="statisticChart" class="charts chart-bar" style="width: 100%; height: 100%"></div>
      <Spin fix v-if="loading || !tableData.length">
...
import { getReportChart } from '@/api/report'
...
export default {
  ...
  methods: {
    ...
    getChart () {
      ...
      getReportChart(ajaxData).then((res) => {
        let statisticsDatas = res.data.statisticsDatas
  ...
  mounted () {
    this.getChart()
    this.$nextTick(() => {
      this.chartDom = echarts.init(this.$refs.statisticChart, 'tdTheme')
    })
  }
}
</script>
