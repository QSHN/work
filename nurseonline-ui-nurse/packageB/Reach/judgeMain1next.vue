		...
		<scroll-view class="nav-right" scroll-y :scroll-top="scrollTop" @scroll="scroll" :style="'height:'+height+'px'" scroll-with-animation>
			<view v-for="(foods,index) in classifyData" :key="index" class="box">
				<view :id="i==0?'first':''" class="nav-right-item" v-for="(item,i) in foods.foods" :key="i" @click="cart(item.name)">
					<view>{{item.name}}<text class="iconfont">&#xe6f8;</text></view>
				</view>
			</view>
		</scroll-view>
		<view  class="saveJudge">
			<button type="primary color6d8bf9" @tap="toJudgeMain">保存</button>
		</view>
	</view>
</template>

<script>
	import classifyData from '../../static/common/departmentData/main1next.js';
	export default {
		data() {
			return {
				categoryList: [],
