<template>
	<view class="content flex-column align-center">
		...
		<view class="login-select justify-center">
			<view :class="mobileLogin ? 'selected' : ''" @tap="changeLogin('mobile')">快捷登录</view>
			<view :class="mobileLogin ? '' : 'selected'" @tap="changeLogin('pass')">密码登录</view>
		</view>
		<!-- <view  @tap="makeServicePhone"> -->
			<button class="servicephone-box flex-column  font-blue" open-type="contact">
				<text class="iconfont color6e8cfa">&#xe609;</text>
				<text class="color6e8cfa servicephone-text">咨询</text>
				<text class="color6e8cfa servicephone-text">客服</text>
			</button>	
		<!-- </view> -->
		<view class="login-area">
			<view class="login-box">
				...
				<template v-else>
					<view class="uni-flex justify-content-space">
						<input type="text" v-model="user.username" placeholder="请输入用户名/手机号" placeholder-class="holder"/>
						<text v-if='showClearUserName' @tap='clearUserName' class="iconfont mr20">&#xe63e;</text> <!-- 一键清除 -->
					</view>
				</template>
			</view>
			<view class="login-box flex-msg">
				...
				<template v-else>
					<input v-if="showPsw" type="password" v-model="user.password" placeholder="请输入密码" placeholder-class="holder"/><!-- 密码不可见 -->
					<input v-else type="text" v-model="user.password"  placeholder="请输入密码" placeholder-class="holder"/><!-- 密码可见 -->
					<view @tap='clearPsw' v-if="showClearPsw"><text class="iconfont mr20">&#xe63e;</text></view><!-- 一键清除 -->
					<view v-if="rememberPsw"><!-- 默认隐藏 -->
						<text @tap="changeShowPsw" v-if="showPsw" class="iconfont mr20">&#xe634;</text> 
						<text @tap="changeShowPsw" v-else class="iconfont mr20">&#xe633;</text>
					</view>
				</template>
			</view>
		</view>
		<view class="register-box uni-flex justify-content-space width500">
			<view class="uni-item" v-if="!mobileLogin">
				<checkbox value="remember" :checked="checkPsw" @tap="checkPswSelect"/>
				<text class="ml20 font-size24">记住密码</text>
			</view>
		</view>
		<view class="btn-area"  @tap="login">
			<button  type="primary bgc-6e8cfa" :class="loginDisabled?'bgc999':'bgc6e8cfa'" :disabled="loginDisabled">登录</button>
		</view>
		<view class="register-box uni-flex justify-content-space width500 mt40-im" v-if="!mobileLogin" >
			<view class="uni-item" @tap="changePwd">
				<text class="fc-normal font-size26">忘记密码</text>
			</view>
			<view class="uni-item" @tap="reg">
				<text class="fc-normal font-size26">立即注册</text>
			</view>
		</view>
		<view class="width100p text-alignC color666 font-size24 mt48" v-else>未注册手机验证后自动注册</view>
		<view>
			<view class="text-alignC color666 font-size24 mt40 width500 ">---------- 第三方登录 ----------</view>
			<button  open-type="getUserInfo" @tap.stop.prevent="weixinLogin" class=" bgc-fff wechatLoginBtn"><text class="iconfont color6e8cfa font-size50">&#xe707;</text></button>
		</view>
		...
		<view class="swiper-text">
			<view class="swiper-text-cn mt80" style="color:#595757">V{{versionNumber}}</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods:{
			...
			async getCode() {
			        ...
			        let res = await this.$api.getCode(this.mobile.mobile);
					uni.hideLoading();
					let backData=JSON.parse(res["data"]).data
			},
			async login(){
				this.$store.state.wechatBindRes=false//不跳转到微信绑定验证码校验页面
				if(!this.checked){
					uni.showToast({
						title:'请阅读并勾选服务协议',
						icon:'none'
			...
			async weixinLogin(){
				...
				uni.login({
				  ...
				  success: async (loginRes) =>{
					// 获取用户信息
					let getUserInfo_pro=new Promise((resolve,reject)=>{
						uni.getUserInfo({
						  provider: 'weixin',
						  success: (infoRes)=> {
							console.log(infoRes);
			...
			makeServicePhone(){
				uni.makePhoneCall({
					phoneNumber:this.$store.state.servePhone//转wx-contact
				})
			}
		}
	}
