...
export function getDics(dics){
  console.log("util getDics", dics);
  // let dics = {...dic};
  let dicList = {};
  for(let key in dics){
    let str = "";
  ...
  for(let item in dicList){
    let key = dicList[item];
    console.log("kkk", dics[item]);
    if(!dics[item] || JSON.stringify(dics[item]) === "{}"){
