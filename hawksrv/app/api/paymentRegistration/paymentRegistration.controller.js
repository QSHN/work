...
async function create(req, res, next) {
    L('Calling API PaymentRegistration.create', req.body);
    ...
    try {
        ...
        let apply = await PaymentRegistration.create(obj);
        if (obj.visitId) {
            let vData = await TerminalVisit.findById(obj.visitId);
            vData.task.forEach(v => {
                if (v.name == '收款登记') v.list.push(apply)
            })
        ...
        if (obj.approver) {
            obj.approver.forEach(v => {
                pushMsg({
                    userId: v.uid,
                    category: '收款登记',
                    content: `亲，您${req.user.sub !== v.uid ? '的下属' : ''}提交了收款登记，请尽快审批`,
                    title: '收款登记',
                    applyId: apply._id
                });
            })
...
async function getById(req, res, next) {
    L('Calling API PaymentRegistration.getById', req.params);
    if (!req.params.id) {
        next(new errors.BadRequest('invalid PaymentRegistration id'));
        return;
    }
    try {
        let result = await PaymentRegistration.findById(req.params.id);
...
async function query(req, res, next) {
    L('Calling API PaymentRegistration.query');
    ...
    try {
        ...
        if (val.page && val.limit) {
            result = await PaymentRegistration.find(filter).sort({createdAt: -1})
                .limit(parseInt(val.limit))
                .skip(val.limit * (val.page - 1));
        } else {
            result = await PaymentRegistration.find(filter);
        }
        res.status(200).json(result);
    } catch (error) {
        L(error);
    ...
    try {
        ...
        if (obj.status === '已审批') {
            ...
            pushMsg({
                ...
                category: '收款登记',
                content: `亲，您有一张收款登记单需要录入到财务系统，请尽快查看`,
                title: '收款登记',
                applyId: _id
            })
            delete obj.finance
        ...
        await PaymentRegistration.update({_id, companyId}, obj)
        if (obj.status !== '待审批') {
            pushMsg({
                userId: obj.creator,
                category: "收款登记",
                content: `您的收款登记${obj.status === '已审批' ? '已审批' : '被驳回'}`,
                title: "收款登记审批结果",
                applyId: _id,
            });
        }
...
async function deletePaymentRegistration(req, res, next) {
    L('Calling API deletePaymentRegistration.delete', req.params);
    if (!req.params.id) {
        next(new errors.BadRequest('invalid PaymentRegistration id'));
        return;
    }
    try {
        await PaymentRegistration.deleteOne({_id: req.params.id})
        res.status(200).end();
    } catch (error) {
        L(error);
        next(new errors.BadRequest(error.message));
