...
async function create(req, res, next) {
    L('Calling API procurement.create', req.body);
    ...
    try {
        ...
        let apply = await procurement.create(obj);
        ...
        for (var i = 0;i < obj.approver.length;i ++){
            ...
            pushMsg({
                ...
                category:"采购",
                content: myuser.name+"向您提交采购申请",
                title:myuser.name+"给您提交采购",
            });
        }
        obj.sendover.forEach((v) => {
            pushMsg({
                userId: v,
                category: "采购",
                content: `${myuser.name}提交了采购申请`,
                title: `${myuser.name}提交了采购申请`,
                applyId: apply._id,
            });
        });
...
async function getById(req, res, next) {
    L('Calling API procurement.getById', req.params.id);
    ...
    try {
        ...
        let a = await procurement.findById(aId, filteringItems);
...
async function query(req, res, next) {
    ...
    try {
        ...
        let result = await procurement.find(filters, filteringItems).sort({createdAt:-1});
...
async function update(req, res, next) {
    L('Calling API procurement.update', req.params.id, req.body);
    let aId = req.params.id;

    if (!aId) {
        throw new Error('invalid parameter for API procurement.update');
    }
    ...
    try {
        ...
        let a = await procurement.findByIdAndUpdate(aId, data, {new: true});

        pushMsg({
            userId:a["applicant"],
            category:"采购",
            content: `你的采购${a["status"] == "审批通过" ? '已通过' : '被拒绝'}`,
            title:"采购提醒通知",
            applyId: a._id,
        });
        ...
        if (a["status"] == "审批通过") {
            pushMsg({
                ...
                category: '采购',
                content: `亲，您有一张采购单需要录入到财务系统，请尽快查看`,
                title: '采购单',
                applyId: a._id
            })
...
async function updatemysender(req, res, next) {
    L('Calling API procurement.updatemysender', req.params.id, req.body);
    let aId = req.params.id;

    if (!aId) {
        throw new Error('invalid parameter for API procurement.updatemysender');
    }

    try {
        let a = await procurement.findByIdAndUpdate(aId, req.body);
        res.status(200).json(a);
    } catch (error) {
        L(error);
        next(new errors.BadRequest(error.message));
