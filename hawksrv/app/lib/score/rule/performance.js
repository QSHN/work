...
const performance = require('../../../api/performance/performance.model')
            ...
            cron: {
                ...
                run: async function () {
                    ...
                    let list = await ScoreRuleModel.find({
                        "ruleModule": "performance",
                        "rules.rule": "performanceCount",
                        "run": true
                    })
                    ...
                    list.forEach(async item => {
                        item.rules.forEach(async cr => {
                            ...
                            if (curr_rule.rule == "performanceCount") {
                                ...
                                item.users.forEach(async user => {
                                    let visit_list = await performance.find({
                                        creator: user,
                                        status: '已确认',
                                    ...
                                    if (count >= curr_rule.unitValue) {
                                        ...
                                        let score = {
                                            ...
                                            // ref: visit,
                                            detail: date.to.toHawkDateString() + " 完成绩效报告次数",
                                        };
                                        ...
                                        if (!list.length) {
                                            ...
                                            pushMsg({
                                                userId: user,
                                                category: "工作考核",
                                                content: '亲，您因为完成绩效报告次数，被奖励：' + curr_rule.more.add.value + '分',
                                                title: "完成绩效报告次数",
                                            });
                                        }
                                    } else {
                                        ...
                                        score = {
                                            ...
                                            // ref: visit,
                                            detail: date.to.toHawkDateString() + " 未完成绩效报告次数",
                                        };
                                        let list = await Score.find(score);
                                        if (!list.length) {
                                            await Score.create(score);
                                            pushMsg({
                                                userId: user,
                                                category: "工作考核",
                                                content: '亲，您因为未完成绩效报告次数，被扣分：' + curr_rule.more.sub.value + '分',
                                                title: "未完成绩效报告",
                                            });
                                        }
                                    }
...
// ScoreRule.rules.performanceCount.cron.run() //测试用
