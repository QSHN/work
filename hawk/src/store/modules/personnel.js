...
const getters = {
    ...
    userPermissions: state => uId => {
        ...
        let basic_perms = [
          '我的考勤', '任务管理', '日报管理', '通讯目录', '实时查岗',
          '工作轨迹', '费用报销', '绩效结算', '填写汇报', '通知公告',
          '客户管理', '拜访管理', '扫名片', '考勤管理', '用车申请',
        ...
        let perms = JSON.parse(JSON.stringify(u.privilege)) || [],
        ...
        let list = perms.length ? perms : basic_perms;
        if (u.accountType === '财务') {
          list.push('报表')
          list.push('凭证')
          list.push('科目余额')
