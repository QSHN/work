...
export default {
    ...
    data() {
        return {
            perms: {
                ...
                advanced: {
                    title: '高级权限',
                    permission: ['权限管理', '组织构架', '绩效设置', '经营分析'],
    ...
    computed: {
        ...
        all_selected() {
            return this.selected.length == ['basic', 'administration', 'advanced'].reduce((accum, item) => { return accum + this.perms[item].permission.length}, 0);
        }
    },
    ...
    methods: {
        ...
        set_selected(options) {
            ...
            if (groups) this.selected = groups.filter(g => ['basic', 'administration', 'advanced'].includes(g))
        ...
        selectAll(checked) {
            ...
            this.selected = ['basic', 'administration', 'advanced'].reduce((accum, item) => { return accum.concat(this.perms[item].permission)}, []);
        }
    }
}
</script>
