...
<script>
    ...
    import {addMarker,  getAddress} from 'lib/amap';
                ...
                if (device.platform == "Android" || device.platform == "iOS") {
                    if (cordova.plugins.aMapLocation) {
                        ...
                        cordova.plugins.aMapLocation.getCurrentPosition(null,  (response) => {
                            ...
                            self.result.location = self.myPosition
                            if (!response.address) {
                                getAddress(response.longitude, response.latitude).then(address => {
                                    self.myPosition.address = address
                                    self.result.location = self.myPosition
                                    self.$forceUpdate()
                                })
                            }
            ...
            save(item,index){
                ...
                axios.put('api/visit/' + this.visit._id, dic).then(() => {
                    ...
                    if (index == 2){
                        ...
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '完成' + this.visit.type + '拜访'})
                        }
                    }else if (index == 0){
                        ...
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '开始走访'})
                        }
                    }
                    setTimeout(function(){
                        $preloader.hide();
