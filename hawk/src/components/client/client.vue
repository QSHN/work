    ...
    export default {
        ...
        methods: {
            ...
            confirmSave(etype) {
                ...
                sync(path, this.client).then(resp => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `保存${etype}资料`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    console.error(err);
            ...
            remove() {
                ...
                let etype = this.classify == '客户资料' ? '客户' : '商家';
                $confirm('确定要删除该'+ etype +'吗？', '操作确认', () => {
                    $preloader.show();
                    axios.delete(`api/client/${this.client._id}`).then(resp => {
                        $preloader.hide();
                        this.$f7router.back();
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除该${etype}`})
                        }
                    }).catch(err => {
                        $preloader.hide();
