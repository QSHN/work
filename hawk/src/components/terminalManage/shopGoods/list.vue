<template>
    <f7-page @page:reinit="resetLoad">
        <f7-navbar title="铺货上报清单" back-link>
            <f7-nav-right>
        ...
        <div class="pc_content">
            <div class="pc_layer">
                <div class="list_item" v-for="(v, i) in list" @click="$f7router.navigate('/terminalManage/shopGoods/detail', { props: { detail: v }})">
    ...
    methods: {
        ...
        addFun () {
            if (this.visitData && this.visitData.visitor !== this.myUserId) {
                ...
                this.$f7router.navigate('/terminalManage/shopGoods/', {
                    props: {
        ...
        searchFun () {
            this.$f7router.navigate('/terminalManage/shopGoods/search', {
                props: {
                    search: (val) => {
                        this.filter = {...val}
        ...
        async load (state, cb) {
            ...
            await axios.post('api/shopGoods/list', this.filter).then(res => {
                this.identifier++;
