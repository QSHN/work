<template>
    <f7-page  @page:reinit="load">
        <f7-navbar title="综合上报详情" back-link>
            <f7-nav-right>
                <f7-link href="#" v-if="formData.approver[0].status === '待审批'" @click="selectPicker">操作</f7-link>
            </f7-nav-right>
        </f7-navbar>
        ...
        <div class="p_content">
            <div class="pc_layer">
                <div class="commodity_t">
                    <div class="title">
                        {{formData.name}}
                        <span :class="formData.status == '已批准' ? 'tip success' : formData.status == '被驳回' ? 'tip error' : 'tip'">{{formData.status}}</span>
                    </div>
                ...
                <div class="commodity" v-if="formData.commodity.length">
                    ...
                    <div class="item" v-for="(v, i) in formData.commodity">
                        ...
                        <p>库存单价： {{v.price}} 元 / {{v.commodity.unit}}</p>
                        <p>库存数量： {{v.inventory}} {{v.commodity.unit}}</p>
                        <p>库存金额： {{v.price * v.inventory}} 元</p>
                        <p>销售单价： {{v.price}} 元 / {{v.commodity.unit}}</p>
                    ...
                    <div class="all_price">
                        <span>库存合计金额：</span>
                        <p>{{formData.commodity.reduce((all, v) => all += v.price * v.inventory, 0)}} 元</p>
                    </div>
                    <div class="all_price">
                        <span>销售合计金额：</span>
                        <p>{{formData.commodity.reduce((all, v) => all += v.price * v.number, 0)}} 元</p>
                    </div>
                </div>
                ...
                <div class="list-item photo-item" style="height: 115px !important;">
                    <span class="item-icon">*</span>
                    <label>审批人</label>
                    <div class="clearfix">
...
export default {
    ...
    methods: {
        moment,
        load () {
            axios.get('api/salesIntegrated/id/' + this.formData._id).then(res => {
                if (res.data) {
        ...
        reject () {
            $prompt('请输入驳回原因', '驳回原因', (name) => {
                ...
                axios.post('api/salesIntegrated', data).then(res => {
                    $preloader.hide();
                    $alert('驳回成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回综合上报`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('驳回失败！')
        ...
        agree () {
            $confirm('确定同意该综合上报吗？', () => {
                ...
                axios.post('api/salesIntegrated', data).then(res => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `通过综合上报`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('审批失败！')
        ...
        update () {
            this.$f7router.navigate('/terminalManage/salesIntegrated/', {
                props: {
                    uData: this.formData
                }
            })
        ...
        deleteFun () {
            $confirm('确定删除该综合上报吗？', () => {
                $preloader.show();
                axios.delete('api/salesIntegrated/'+ this.formData._id).then(res => {
                    $preloader.hide();
                    $alert('删除成功！', () => {
                        this.$f7router.back()
                    })
