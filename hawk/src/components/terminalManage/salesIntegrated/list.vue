<template>
    <f7-page @page:reinit="resetLoad">
        <f7-navbar title="上报记录" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                ...
                <div class="list_item" v-for="(v, i) in list" @click="$f7router.navigate('/terminalManage/salesIntegrated/detail', { props: { detail: v }})">
                    <div class="title">
                        {{v.name}}
                        <span :class="v.status == '已批准' ? 'tip success' : v.status == '被驳回' ? 'tip error' : 'tip'">{{v.status}}</span>
                    </div>
                    <div class="other">
                    ...
                    <p>上报单号： {{v.coding}}</p>
                    <p>上报sku数： {{v.commodity.length}}</p>
                    <p>销售金额： {{v.commodity.reduce((all, v) => all += v.price * v.number, 0)}}元</p>
                </div>
...
export default {
    ...
    methods: {
        ...
        searchFun () {
            this.$f7router.navigate('/terminalManage/salesIntegrated/search', {
                props: {
                    search: (val) => {
                        this.filter = {...val}
        ...
        async load (state, cb) {
            ...
            await axios.post('api/salesIntegrated/list', this.filter).then(res => {
                this.identifier++;
