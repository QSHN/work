<template>
    <f7-page  @page:reinit="load">
        <f7-navbar title="销量上报详情" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                <div class="commodity_t">
                    ...
                    <p>销售日期：{{moment(formData.time).format('YYYY-MM-DD HH:mm:ss')}}</p>
                ...
                <div class="commodity" v-if="formData.commodity.length">
                    ...
                    <div class="item" v-for="(v, i) in formData.commodity">
                        ...
                        <p>单价： {{v.price}} / {{v.commodity.unit}}</p>
...
export default {
    ...
    methods: {
        moment,
        load () {
            axios.get('api/salesVolume/id/' + this.formData._id).then(res => {
                if (res.data) {
        ...
        reject () {
            $prompt('请输入驳回原因', '驳回原因', (name) => {
                ...
                axios.post('api/salesVolume', data).then(res => {
                    $preloader.hide();
                    $alert('驳回成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回销售上报`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('驳回失败！')
        ...
        agree () {
            $confirm('确定同意该销量上报吗？', () => {
                ...
                axios.post('api/salesVolume', data).then(res => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `通过销售上报`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('审批失败！')
        ...
        update () {
            this.$f7router.navigate('/terminalManage/salesVolume/update', {
                props: {
                    uData: this.formData
                }
            })
        ...
        deleteFun () {
            $confirm('确定删除该销量上报吗？', () => {
                $preloader.show();
                axios.delete('api/salesVolume/'+ this.formData._id).then(res => {
                    $preloader.hide();
                    $alert('删除成功！', () => {
                        this.$f7router.back()
                    })
