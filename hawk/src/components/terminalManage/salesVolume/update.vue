<template>
    <f7-page>
        <f7-navbar title="新增销量" back-link></f7-navbar>
            ...
            <div class="pc_layer">
                ...
                <div class="commodity" v-if="formData.commodity.length">
                    <div class="item" v-for="(v, i) in formData.commodity">
                        ...
                        <p>销售： {{v.number}} {{v.commodity.unit}}</p>
    ...
    data () {
        return {
            ...
            formData: {
                ...
                approver: [],
                status: '待审批',
                coding: `XL${Date.parse(new Date())}${Math.floor(Math.random () * 900) + 100}`
            },
            clientGrade: '',
    ...
    methods: {
        ...
        submit () {
            if (!this.formData.name) {
                ...
                this.formData.allPrice = this.getAllPrice()
                axios[this.uData ? 'post' : 'put']('api/salesVolume/', this.formData).then(res => {
                    $alert('销量上报成功', () => {
                       this.$f7router.back();
                    });
                    if (!this.uData) {
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '销量上报'})
                        }
                    }
                }).catch(err => {
