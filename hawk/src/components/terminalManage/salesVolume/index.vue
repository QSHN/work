<template>
    <f7-page @page:reinit="resetLoad">
        <f7-navbar title="销量上报" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                ...
                <div class="list_item" v-for="(v, i) in list" @click="$f7router.navigate('/terminalManage/salesVolume/detail', { props: { detail: v }})">
                    <div class="title">
                        {{v.name}}
...
export default {
    ...
    methods: {
        ...
        searchFun () {
            this.$f7router.navigate('/terminalManage/salesVolume/search', {
                props: {
                    search: (val) => {
                        this.filter = {...val}
        ...
        async load (state, cb) {
            ...
            await axios.post('api/salesVolume/list', this.filter).then(res => {
                this.identifier++;
