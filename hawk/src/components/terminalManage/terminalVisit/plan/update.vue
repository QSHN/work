<template>
    <f7-page>
        <f7-navbar title="新增计划" back-link></f7-navbar>
        ...
        <div class="p_content">
            <div class="pc_layer">
                ...
                <div class="list-item clear-float">
                    ...
                    <label>开始时间：</label>
                    <div style="float: right">
                        <date-picker v-model="formData.planStartTime" type="datetime"
                                     :editable="false"
                                     class="date-picker"
                                     width="150px"
                                     :confirm="true"
                ...
                <div class="list-item clear-float">
                    ...
                    <label>结束时间：</label>
                    <div style="float: right">
                        <date-picker v-model="formData.planEndTime" type="datetime"
                                     :editable="false"
                                     class="date-picker"
                                     width="150px"
                                     :confirm="true"
                ...
                <div class="list-item clear-float">
                    ...
                    <label>拜访客户：</label>
                    <div style="float: right">
                        <div class="list_item_btn" style="float: right;" @click="selectCricuit">从线路添加</div>
                        <div class="list_item_btn" style="float: right;" @click="selectClient">从列表添加</div>
                    </div>
                </div>

                <div class="commodity" v-if="formData.client.length">
                    <div class="item" v-for="(v, i) in formData.client">
                        <div class="title">
                        ...
                        <p>类型： {{v.type || '无'}}</p>
                        <p>等级： {{v.grade || '无'}}</p>
                        <img src="../../../../assets/images/icon_prune_gray@2x.png" @click="formData.client.splice(i, 1)" />
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            selectCricuit () {
                this.$f7router.navigate('/terminalManage/terminalVisit/circuit', {
                    props: {
                        choose: (val) => {
            ...
            selectClient () {
                this.$f7router.navigate('/selectListMore', {
                    props: {
                        which: {
                            name: '客户',
                            type: 'name'
                        },
                        sData: this.formData.client,
                        url: 'client/',
                        choose: (val) => {
                            this.formData.client = val
                            this.$f7router.back()
            ...
            submit () {
                if (!this.formData.visitor) {
                    $alert('请选择拜访人')
                } else if (!this.formData.planStartTime) {
                    $alert('请选择开始时间')
                } else if (!this.formData.planEndTime) {
                    $alert('请选择截止时间')
                } else if (!this.formData.plan) {
                    $alert('请输入计划内容')
                } else if (!this.formData.client.length) {
                    $alert('请选择拜访客户')
                } else {
                    axios[this.uData ? 'post' : 'put']('api/terminalVisit/plan/', this.formData).then(res => {
                        $alert('新增计划成功', () => {
                            this.$f7router.back()
                        });
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `终端拜访-计划拜访`})
                        }
                    }).catch(err => {
