<template>
    <f7-page @page:reinit="resetLoad">
        <f7-navbar title="计划拜访" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                <div class="list_item" v-for="(v, i) in list" @click="$f7router.navigate('/terminalManage/terminalVisit/plan/detail', { props: { detail: v }})">
                    <div class="title">
                        {{who(v.visitor) && who(v.visitor).name}}
                    </div>
                    <div class="other">
                        <div class="time">{{moment(v.createdAt).format('YYYY-MM-DD HH:mm:ss')}}</div>
                        <div class="name">{{who(v.creator) && who(v.creator).name}}</div>制定计划
                    </div>
                    <p>开始日期： {{moment(v.planStartTime).format('YYYY-MM-DD HH:mm:ss')}}</p>
                    <p>截止日期： {{moment(v.planEndTime).format('YYYY-MM-DD HH:mm:ss')}}</p>
...
<script>
    ...
    export default {
        ...
        methods: {
            moment,
            addFun () {
                this.$f7router.navigate('/terminalManage/terminalVisit/plan/update')
            },
            searchFun () {
                this.$f7router.navigate('/terminalManage/terminalVisit/plan/search', {
                    props: {
                        search: (val) => {
                            this.filter = {...val}
                            if (val.startTime) this.filter.planStartTime = new Date(val.startTime).toHawkDateString() + ' 00:00:00.000'
                            if (val.endTime) this.filter.planEndTime = new Date(val.endTime).toHawkDateString() + ' 23:59:59.999'
                            this.filter.page = 1
            ...
            async load (state, cb) {
                ...
                await axios.post('api/terminalVisit/plan/list', this.filter).then(res => {
                    this.identifier++;
