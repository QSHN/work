<template>
    <f7-page>
        <f7-navbar title="拜访总结" back-link></f7-navbar>
    ...
    export default {
                    ...
                    if (!this.formData.summary) {
                        ...
                        axios.post('api/terminalVisit/visit/', data).then(res => {
                            ...
                            if (cordova.plugins.aMapLocation) {
                                this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `终端拜访成功`})
                            }
                        }).catch(err => {
        ...
        mounted() {
            if (this.visitData && this.visitData.task.length) {
                this.visitData.task.forEach(v => {
                    if (v.name === '拜访总结') {
                        if (v.list.length) {
                            this.formData = v.list[0]
                        } else {
                            this.formData = {
