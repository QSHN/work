<template>
    <f7-page  @page:reinit="load">
        <f7-navbar title="线路详情" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                <div class="commodity_t">
                    ...
                    <p>创建人：{{who(formData.creator) && who(formData.creator).name}}</p>
                    <p>创建日期：{{moment(formData.createdAt).format('YYYY-MM-DD HH:mm:ss')}}</p>
                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            moment,
            load () {
                axios.get('api/terminalVisit/circuit/id/' + this.formData._id).then(res => {
                    if (res.data) {
            ...
            deleteFun () {
                $confirm('确定删除该线路吗？', () => {
                    $preloader.show();
                    axios.delete('api/terminalVisit/circuit/'+ this.formData._id).then(res => {
                        $preloader.hide();
                        $alert('删除成功！', () => {
                            this.$f7router.back()
                        })
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除终端拜访线路`})
                        }
                    }).catch(err => {
                        $preloader.hide();
                        $alert('删除失败！')
