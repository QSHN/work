<template>
    <f7-page>
        <f7-navbar title="库存上报" back-link>
            <f7-nav-right v-if="!visitData">
                <f7-link href="#" @click="$f7router.navigate('/terminalManage/inventory/list')">上报记录</f7-link>
            </f7-nav-right>
        </f7-navbar>
    ...
    data () {
        return {
            formData: {
                ...
                photos: [],
                des: '',
                coding: `PR${Date.parse(new Date())}${Math.floor(Math.random () * 900) + 100}`,
            },
            clientGrade: ''
        }
    },
        ...
        submit () {
            if (!this.formData.name) {
                ...
                if (!this.formData.photos.length) {
                    ...
                    $confirm('您未拍照，是否继续上报', '提示', () => {
                        axios.put('api/inventory/', this.formData).then(res => {
                            $alert('库存上报成功', () => {
                                if (this.visitData) {
                                    this.$f7router.back()
                                } else {
                                    this.$f7router.navigate('/terminalManage/inventory/list')
                                }
                            })
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '库存上报'})
                        }).catch(err => {
