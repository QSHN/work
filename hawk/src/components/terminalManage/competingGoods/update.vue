    ...
    methods: {
        ...
        submit () {
            if (!this.formData.name) {
                ...
                axios[this.uData ? 'post' : 'put']('api/competingGoods/', this.formData).then(res => {
                    $alert('竞品上报成功', () => {
                       this.$f7router.back();
                    });
                    if (!this.uData) {
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '竞品上报'})
                        }
                    }
                }).catch(err => {
