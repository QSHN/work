        ...
        methods: {
            ...
            recall(){
                ...
                axios.put("api/reportdata/"+self.reportData._id, this.reportData).then(resp => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `撤回汇报`})
                    }
                }).catch(err => {
                    $preloader.hide();
            ...
            submit() {
                ...
                axios.post(path, this.reportData).then(resp => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '提交汇报'})
                    }
                }).catch(err => {
                    $preloader.hide();
                    if (err.response.data.errorCode == 501) {
