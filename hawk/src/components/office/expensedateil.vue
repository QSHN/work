                ...
                if (this.approver.lister) {
                    ...
                    axios.put('api/expensemgr/' + aId, {status, finance}).then(() => {
                        ...
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '审批通过报销单'})
                        }
                    }).catch(error => {
                        console.error(error);
            ...
            reject() {
                ...
                $dialog.prompt('请填写拒绝原因', '拒绝原因', val => {
                    ...
                    axios.put('api/expensemgr/' + aId, {status, notes: val}).then(() => {
                        ...
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '拒绝报销单'})
                        }
                    }).catch(error => {
                        console.error(error);
                        this.nobusy('提交失败，请稍后再试', '操作失败');
