    ...
    <f7-page @page:reinit="load">
        ...
        <div v-else>
            ...
            <table class="info_con" cellspacing="0">
                ...
                <tr v-if="terminalVisit.addArr.length">
                    <td>终端拜访</td>
                    <td>
                        <p v-for="item in terminalVisit.addArr" @click="showDetail(item)">{{item.detail}}
                            加{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="shopGoods.addArr.length">
                    <td>铺货上报</td>
                    <td>
                        <p v-for="item in shopGoods.addArr" @click="showDetail(item)">{{item.detail}}
                            加{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="inventory.addArr.length">
                    <td>库存上报</td>
                    <td>
                        <p v-for="item in inventory.addArr" @click="showDetail(item)">{{item.detail}}
                            加{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="salesVolume.addArr.length">
                    <td>销量上报</td>
                    <td>
                        <p v-for="item in salesVolume.addArr" @click="showDetail(item)">{{item.detail}}
                            加{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="salesIntegrated.addArr.length">
                    <td>综合上报</td>
                    <td>
                        <p v-for="item in salesIntegrated.addArr" @click="showDetail(item)">{{item.detail}}
                            加{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="imageAcquisition.addArr.length">
                    <td>终端形象采集</td>
                    <td>
                        <p v-for="item in imageAcquisition.addArr" @click="showDetail(item)">{{item.detail}}
                            加{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="competingGoods.addArr.length">
                    <td>竞品上报</td>
                    <td>
                        <p v-for="item in competingGoods.addArr" @click="showDetail(item)">{{item.detail}}
                            加{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="inventoryVerification.addArr.length">
                    <td>库存盘点</td>
                    <td>
                        <p v-for="item in inventoryVerification.addArr" @click="showDetail(item)">{{item.detail}}
                            加{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="salesmanage.addArr.length">
                    <td>销售业绩</td>
                    <td>
                        <p v-for="item in salesmanage.addArr" @click="showDetail(item)">{{item.detail}}
                            加{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="performance.addArr.length">
                    <td>绩效报告</td>
                    <td>
                        <p v-for="item in performance.addArr" @click="showDetail(item)">{{item.detail}}
                            加{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
            ...
            <table class="info_con" cellspacing="0">
                ...
                <tr v-if="terminalVisit.subArr.length">
                    <td>终端拜访</td>
                    <td>
                        <p v-for="item in terminalVisit.subArr" @click="showDetail(item)">{{item.detail}}
                            扣{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="shopGoods.subArr.length">
                    <td>铺货上报</td>
                    <td>
                        <p v-for="item in shopGoods.subArr" @click="showDetail(item)">{{item.detail}}
                            扣{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="inventory.subArr.length">
                    <td>库存上报</td>
                    <td>
                        <p v-for="item in inventory.subArr" @click="showDetail(item)">{{item.detail}}
                            扣{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="salesVolume.subArr.length">
                    <td>销量上报</td>
                    <td>
                        <p v-for="item in salesVolume.subArr" @click="showDetail(item)">{{item.detail}}
                            扣{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="salesIntegrated.subArr.length">
                    <td>综合上报</td>
                    <td>
                        <p v-for="item in salesIntegrated.subArr" @click="showDetail(item)">{{item.detail}}
                            扣{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="imageAcquisition.subArr.length">
                    <td>终端形象采集</td>
                    <td>
                        <p v-for="item in imageAcquisition.subArr" @click="showDetail(item)">{{item.detail}}
                            扣{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="competingGoods.subArr.length">
                    <td>竞品上报</td>
                    <td>
                        <p v-for="item in competingGoods.subArr" @click="showDetail(item)">{{item.detail}}
                            扣{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="inventoryVerification.subArr.length">
                    <td>库存盘点</td>
                    <td>
                        <p v-for="item in inventoryVerification.subArr" @click="showDetail(item)">{{item.detail}}
                            扣{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="salesmanage.subArr.length">
                    <td>销售业绩</td>
                    <td>
                        <p v-for="item in salesmanage.subArr" @click="showDetail(item)">{{item.detail}}
                            扣{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
                <tr v-if="performance.subArr.length">
                    <td>绩效报告</td>
                    <td>
                        <p v-for="item in performance.subArr" @click="showDetail(item)">{{item.detail}}
                            扣{{item.score}}分<span v-if="item.status=='撤消'">(已撤消)</span></p>
                    </td>
                </tr>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            terminalVisit() {
                return this.pScore('终端拜访')
            },
            shopGoods() {
                return this.pScore('铺货上报')
            },
            inventory() {
                return this.pScore('库存上报')
            },
            salesVolume() {
                return this.pScore('销量上报')
            },
            salesIntegrated() {
                return this.pScore('综合上报')
            },
            imageAcquisition() {
                return this.pScore('终端形象采集')
            },
            competingGoods() {
                return this.pScore('竞品上报')
            },
            inventoryVerification() {
                return this.pScore('库存盘点')
            },
            performance() {
                return this.pScore('绩效报告')
            },
            salesmanage() {
                return this.pScore('销售业绩')
            },
            custom() {
                return this.pScore('自定义加减')
            },
            isAdmin() {
                return this.myProfile.superadmin || this.myProfile.privilege.filter(item => item == '绩效设置').length
            },
        },
        ...
        methods: {
            ...
            doScoring() {
                let arr = [
                    this.report, this.attendance, this.task,
                    this.daily, this.visit, this.custom, this.terminalVisit,
                    this.shopGoods, this.inventory, this.salesVolume,
                    this.salesIntegrated, this.imageAcquisition, this.competingGoods,
                    this.inventoryVerification, this.performance, this.salesmanage
                ]
                let add = 0
                let sub = 0
                for (let i in arr) {
                    ...
                    $confirm(tip, '操作确认', () => {
                        axios.put('/api/score/scoreStatus/' + item._id, {
                            ...
                            if (cordova.plugins.aMapLocation) {
                                this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `${status == '撤消' ? '撤销' : '恢复'}${item.rule.valueType=='sub'?'扣分':'加分'}`})
                            }
                        }).catch(err => {
                            $alert('操作失败' + err)
                        })
