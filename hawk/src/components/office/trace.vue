    ...
    <f7-page>
        <f7-navbar title="工作轨迹" back-link>
            <f7-nav-right>
                ...
                <f7-link href="#" @click="getDetail">详情</f7-link>
            </f7-nav-right>
        </f7-navbar>
        ...
        >
            <f7-list-input
                ...
                <f7-icon slot="media" fa="user" style="margin-top: 6px;"></f7-icon>
            </f7-list-input>

            <li>
                <div class="item-content item-input item-input-with-value">
                    <div class="item-media">
                        <i class="icon fa fa-calendar" style="margin-top: 6px;"></i>
                    </div>
                    <div class="item-inner">
                        <div class="item-title item-label">日期</div>
                        <div class="item-input-wrap" @click="openDatePicker">
                            <input type="text" class="input-with-value" v-model="date" readonly>
                        </div>
                    </div>
                </div>
            </li>
        ...
        <f7-popup id="trace-details">
            <f7-page>
                ...
                <f7-list style="margin-top:0" id="hawk-trace-list" v-for="(move, index) in recordDetailList" :key="index">
        ...
        methods: {
                    ...
                    this.datepicker = $calendar.create({
                        ...
                        on: {
                            change: function(calendar, value) {
                                self.date = value[0].toHawkDateString();
                                self.$forceUpdate();
                            }
                        },
                        closeOnSelect: true
            ...
            analyzeTrace() {
                ...
                for (let n = 1; n < this.records.length; n++) {
                    ...
                    let time_diff = moment(new_pos.locatetime).diff(moment(pos.locatetime));

                    if (time_diff > 10 * 60000/* 10 min */) {
                ...
                this.recordDetailList = moves
                $preloader.hide()
                this.$nextTick(() => {
                    this.$f7.popup.open('#trace-details')
                })
            },
