    ...
    methods: {
        ...
        approve() {
            ...
            axios.put('api/application/' + aId, {comment, status}).then(() => {
                ...
                if (cordova.plugins.aMapLocation) {
                    this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '审批通过费用报销'})
                }
            }).catch(error => {
                console.error(error);
                this.nobusy('提交失败，请稍后再试', '操作失败');
        ...
        reject() {
            ...
            axios.put('api/application/' + aId, {comment, status}).then(() => {
                ...
                if (cordova.plugins.aMapLocation) {
                    this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '拒绝费用报销'})
                }
            }).catch(error => {
                console.error(error);
                this.nobusy('提交失败，请稍后再试', '操作失败');
