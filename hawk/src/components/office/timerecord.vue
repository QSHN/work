...
export default {
    ...
    methods: {
        ...
        getAuditing (auditing, isClass) {
            ...
            switch (auditing) {
                case '-1': cn = '待审核'; cls += '#fff; width: 47px; height: 18px; line-height: 18px; text-align: center; background:#3385FF;';
                break;
                case '0': cn = '未通过'; cls += '#FF715A;';
                break;
                case '1': cn = '已通过'; cls += '#3385FF;';
                break;
                default: cn = '审核超时-已通过'; cls += '#3385FF;';
                break;
            }
