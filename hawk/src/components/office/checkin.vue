...
<script>
    ...
    import {createMap, addGeoLocation, addMarker, getDistBetween, getVisibleHeight, getAddress} from 'lib/amap';
            ...
            clockin() {
                ...
                let save = () => {
                    ...
                    this.save({userId, time, location, type, workTime, detail, offset, photo, auditing}).then(() => {
                        ...
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: this.checkin.text})
                        }
                    }).catch((error) => {
                        $preloader.hide();
                        console.error(error);
