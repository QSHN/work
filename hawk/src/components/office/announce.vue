<template>
    <f7-page @page:reinit="load">
        ...
        <f7-list style="margin-top:0">
            ...
            <f7-list-item link="#">
                <span slot="title" style="min-width: 100px; display: block"><span style="color:red">* </span>公告范围</span>
                <input
                    slot="after" type="text" :disabled = "stage('readonly')"
                    placeholder="设置发布范围" readonly="readonly"
                    style="text-align:right; width: 100px" :value="employees"
            ...
            >
                <span slot="title"  style="min-width: 100px; display: block"><span style="color:red">* </span>公告截止日</span>
                <input
                    slot="after"
                    type="text"
            ...
            sync(announce).then(() => {
                ...
                if (!draft) {
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '发布通知公告'})
                    }
                }
            }).catch((err) => {
                console.error(err);
