        ...
        methods: {
                ...
                $dialog.prompt('请填写驳回原因', '驳回原因', val => {
                    ...
                    axios.put('api/journalmgr/' + this.item._id, {status,comment}).then(() => {
                        ...
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '驳回日报'})
                        }
                    }).catch(error => {
                        console.error(error);
            ...
            through(){
                ...
                axios.put('api/journalmgr/' + this.item._id, {status}).then(() => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '确认日报'})
                    }
                }).catch(error => {
                    console.error(error);
                    $alert('无法保存数据，请稍后再试', '操作错误');
