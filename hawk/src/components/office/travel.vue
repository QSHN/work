    ...
    methods: {
        ...
        save(entry) {
            ...
            axios({
                ...
                if (cordova.plugins.aMapLocation) {
                    this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '申请出差'})
                }
            }).catch(error => {
                console.error(error);
        ...
        approve() {
            ...
            axios.put('api/application/' + aId, {comment, status}).then(() => {
                ...
                if (cordova.plugins.aMapLocation) {
                    this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `通过出差申请`})
                }
            }).catch(error => {
                console.error(error);
                this.nobusy('提交失败，请稍后再试', '操作失败');
        ...
        reject() {
            ...
            axios.put('api/application/' + aId, {comment, status}).then(() => {
                ...
                if (cordova.plugins.aMapLocation) {
                    this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回出差申请`})
                }
            }).catch(error => {
                console.error(error);
                this.nobusy('提交失败，请稍后再试', '操作失败');
