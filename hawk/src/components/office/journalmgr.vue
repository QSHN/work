    ...
    export default {
        ...
        methods: {
            ...
            save(entry){
                ...
                if (entry.content.length < this.journalTemplate.contentCount){
                    $alert(`日报内容不能少于${this.journalTemplate.contentCount}字`, '操作提醒');
                    return;
                }
                if (entry.plan == ''){
                ...
                if (entry.plan.length  < this.journalTemplate.planCount){
                    $alert(`计划安排不能少于${this.journalTemplate.planCount}字`, '操作提醒');
                    return;
                }
                ...
                axios.post('api/journalmgr', entry).then(resp => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '提交日报'})
                    }
                }).catch(error => {
                    this.isClick = true;
