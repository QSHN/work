<template>
    <f7-page hide-toolbar-on-scroll class="sore_detail_page">
        ...
        <div @click="newData.show = true" style="width: 100%; font-size: 14px; color: #3385FF; height: 40px; line-height: 40px; text-align: center; background: #fff;">添加考核项</div>
        ...
        <div class="footer">
            <div class="audit_con">
                <div @click="deleteScoreRule">删除考核</div>
                <div v-if="isDone" class="audit_pass">已完成</div>
                <div v-else  @click="saveAllScore" class="audit_pass">保存</div>
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            saveAllScore() {
                ...
                axios.post('api/score/scoreRule', this.scoreData).then(resp => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '添加考核规则'})
                    }
                }).catch(err => {
                    // debugger
                    $preloader.hide();
            ...
            deleteScoreRule(){
                ...
                if (cordova.plugins.aMapLocation) {
                    this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '删除考核规则'})
                }
            },
            radio(event) {
