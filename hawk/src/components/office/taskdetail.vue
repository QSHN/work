            ...
            submitSummary(){
                ...
                axios.put(url, task).then(resp => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '完成任务:' + task.content})
                    }
                }).catch(error => {
                    $preloader.hide();
                    console.error(error);
            ...
            remove(){
                ...
                $confirm("确认要取消该任务吗？","确认操作",()=>{
                    ...
                    axios.delete( 'api/task/' + task._id).then(resp => {
                        ...
                        this.$f7router.back();
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `取消任务`})
                        }
                    }).catch(error => {
                        $preloader.hide();
                        console.error(error);
