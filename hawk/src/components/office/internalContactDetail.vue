    ...
    <f7-page @page:reinit="load">
        <f7-navbar title="公司内部联络单" back-link>
            <f7-nav-right>
                ...
                <f7-link href="#" v-if="isReply" @click="reply">回复</f7-link>
            </f7-nav-right>
        </f7-navbar>
        ...
        <div class="p_content">
            <div class="pc_layer">
                <div class="commodity_t">
                    <div class="title">
                        发文人：{{who(formData.creator) && who(formData.creator).name}}
                        <span :class="formData.status == '已审批' ? 'tip success' : formData.status == '被驳回' ? 'tip error' : 'tip'">{{formData.status}}</span>
                    </div>
                    <p>发文日期：{{moment(formData.createdAt).format('YYYY-MM-DD HH:mm:ss')}}</p>
                    <p>发文类型： {{formData.cType}}</p>
                    <p>收文人： {{formData.send && who(formData.send[0]) && who(formData.send[0]).name}}等{{formData.send.length}}人</p>
                ...
                <div class="list-item clear-float textarea-item" v-for="(v, i) in replyList">
                    <span class="item-icon"></span>
                    <label>{{who(v.replyId) && who(v.replyId).name}}回复:</label>
                    <textarea  :value="v.replyContent"
                               readonly>
                    </textarea>
                </div>
    ...
    export default {
        ...
        computed: {
            ...
            isReply () {
                let bool = false
                if (
                    this.formData.status === '已审批' &&
                    this.formData.send.includes(this.myUserId) &&
                    this.formData.reply.filter(v => v.replyId === this.myUserId).length === 0
                ) {
                    bool = true
                }
                return bool
            ...
            replyList () {
                let list = []
                if (this.formData.status === '已审批') {
                   if (this.formData.creator === this.myUserId) {
                       list = this.formData.reply
                    } else if (this.formData.send.includes(this.myUserId)) {
                       list = this.formData.reply.filter(v => v.replyId === this.myUserId)
                   }
                }
                return list
            }
        ...
        methods: {
            ...
            reject () {
                $prompt('请输入驳回原因', '驳回原因', (name) => {
                    ...
                    axios.post('api/internalContact', data).then(res => {
                        $preloader.hide();
                        $alert('驳回成功！', () => {
                            this.$f7router.back()
                        })
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回公司内部联络单`})
                        }
                    }).catch(err => {
                        $preloader.hide();
                        $alert('驳回失败！')
            ...
            agree () {
                $confirm('确定批准该公司内部联络单吗？', () => {
                    ...
                    axios.post('api/internalContact', data).then(res => {
                        ...
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `通过公司内部联络单`})
                        }
                    }).catch(err => {
                        $preloader.hide();
                        $alert('审批失败！')
            ...
            update () {
                this.$f7router.navigate('/office/internalContact/update/', {
                    props: {
                        uData: this.formData
                    }
                })
            ...
            deleteFun () {
                $confirm('确定删除该公司内部联络单吗？', () => {
                    $preloader.show();
                    axios.delete('api/internalContact/'+ this.formData._id).then(res => {
                        $preloader.hide();
                        $alert('删除成功！', () => {
                            this.$f7router.back()
                        })
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除公司内部联络单`})
                        }
                    }).catch(err => {
                        $preloader.hide();
                        $alert('删除失败！')
            ...
            reply () {
                this.$f7router.navigate('/office/internalContact/reply/', {
                    props: {
