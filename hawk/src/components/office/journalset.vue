    ...
    <f7-page no-toolbar style="background: #F8F8F8">
        ...
        <f7-list>
            ...
            <f7-list-item title="提醒员工提交">
                ...
                <f7-toggle :checked="journalTemplate.isRemind"  @toggleChange="change" style="background: #3385FF"></f7-toggle>
            </f7-list-item>
            ...
            <div class="list-item  list-type">
                <label>日报内容字数不得少于(个)</label>
                <div class="list-type-item">
                    <div class="list-type-item-letter">
                        <div class="list-type-item-letter-left" @click="reduce('contentCount')">-</div>
                        <textarea class="list-type-item-letter-center"
                                  style="padding: 0"
                                  v-model="journalTemplate.contentCount"></textarea>
                        <div class="list-type-item-letter-right" @click="add('contentCount')">+</div>
                    </div>
                </div>
            </div>
            <div class="list-item  list-type">
                <label>计划安排字数不得少于(个)</label>
                <div class="list-type-item">
                    <div class="list-type-item-letter">
                        <div class="list-type-item-letter-left" @click="reduce('planCount')">-</div>
                        <textarea class="list-type-item-letter-center"
                                  v-model="journalTemplate.planCount"
                                  style="padding: 0"></textarea>
                        <div class="list-type-item-letter-right" @click="add('planCount')">+</div>
                    </div>
                </div>
            </div>
        </f7-list>
...
<script>
    ...
    export default {
        ...
        methods: {
            reduce(type){
                if (this.journalTemplate[type] <= 0) return;
                this.journalTemplate[type]--;
            },
            add(type){
                if (this.journalTemplate[type] >= 999) return;
                this.journalTemplate[type]++;
            },
            change (val) {
            ...
            save() {
                ...
                sync(path, this.journalTemplate).then(resp => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '设置日报'})
                    }
                }).catch(err => {
                    $preloader.hide();
                    console.error(err);
