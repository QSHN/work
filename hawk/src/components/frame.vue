        ...
        <f7-navbar :title="curTabTitle">
            <f7-nav-right v-if="showTabs">
                <f7-link v-if="curTab == 'home'" href="/chart/mystats/"
                         style="margin-right: -10px; padding: 0 10px; height: 24px; line-height: 24px; color: #fff; background: #3385FF; border-top-left-radius: 12px; border-bottom-left-radius: 12px; font-size: 12px;">
...
export default {
    ...
    methods: {
        ...
        pushPlugin(){
            ...
            //     axios.put('/api/notification/uploadRegistrationID', {
            ...
            if (device.platform === "Android" || device.platform === "iOS") {
                let t1 = setInterval(()=>{
                    ...
                    if(window.xgPushToken){
                        axios.put('/api/notification/uploadRegistrationID', {
                            registrationID: window.xgPushToken,
                            pushMode: 'xgpush'
                        }).catch(error => {
                            console.error(error);
                        })
        ...
        mandatoryUpdate() {
            try {
                ...
                let appUpdata = function (name) {
                    ...
                    return function (dialog, e) {
                        ...
                        if (name === "下次再说") {
                            ...
                            cordova.InAppBrowser.open("https://itunes.apple.com/cn/app/id1465579230", target, options);
                        } else if (name === "退出应用") {
                ...
                if (device.platform === "Android") {  //android
                    // 设置接口地址
                    // isWifiOnly 默认 false
                    update.init(CONFIG.server.tradeApiUrl+'_backstage/latestAppVersion', false, CONFIG.versionCode);
                    // 手动检查
                    update.manualCheck();
                    // 自动检查
                    update.check();
                } else if (device.platform === "iOS") {  //ios
                    axios.get(CONFIG.server.tradeApiUrl+ '_backstage/latestAppVersion?versionCode=' + CONFIG.versionCode).then(res=>{
