<template>
    <f7-page>
        <f7-navbar title="客户欠费通知" back-link></f7-navbar>
    ...
    export default {
        ...
        methods: {
            ...
            submit () {
                ...
                axios[this.fromData ? 'post' : 'put']('api/clientNotification/', ajaxData).then(res => {
                    $alert('客户欠费通知人员列表更新成功')
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `更新客户欠费通知人员列表`})
                    }
                }).catch(err => {
                    console.error(err);
                    this.nobusy('客户欠费通知人员列表');
                });
            },
            load () {
                axios.post('api/clientNotification/list').then(res => {
