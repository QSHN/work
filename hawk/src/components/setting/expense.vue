            ...
            $prompt('请输入报销类型', '新建报销类型', (name) => {
                ...
                axios[this.isCreate ? 'put' : 'post']('/api/expenseset/', {type: type}).then(resp=>{
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `新建报销类型`})
                    }
                }).catch(err => {
                    console.error(err);
                });
        ...
        deleteType(index) {
            $confirm('您确定要删除该报销类型吗？', () => {
                ...
                axios.post('/api/expenseset/', {type: type}).then(resp => {
                    ...
                    this.isCreate = false
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除报销类型`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('操作失败！')
