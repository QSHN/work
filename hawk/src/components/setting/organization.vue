<template>
    <f7-page @page:reinit="load">
        ...
        <f7-list class="search-list hawk-org" v-if="!searchVal">
            <f7-list-item>
        ...
        <f7-list class="search-list search-all-list hawk-org" v-show="searchVal.length">
            ...
            >
                <div slot="inner" class="hawk-org-user-container">
                    ...
                    <div class="hawk-org-user-title">{{ v.job || '职位待定' }}<span v-if="v.status == false" class="invalid-user">已停用</span></div>
                    <div class="hawk-org-user-admin" v-if="department.admin.includes(v._id)">管理员</div>
                    <div class="hawk-org-user-admin" v-if="isSuperAdmin(v._id)">超级管理员</div>
                </div>
                <f7-link @click="edit_user(v._id)">
                    <img :src="edit_btn_bg" style="width:23px; height:23px">
                </f7-link>
            </f7-list-item>
        </f7-list>
        ...
        delete_users(cb) {
            ...
            axios.delete('api/users?id=' + JSON.stringify(uList)).then(resp => {
                ...
                if (cordova.plugins.aMapLocation) {
                    this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除员工`})
                }
            }).catch(err => {
                console.error(err);
                $alert('无法删除员工', '操作错误');
        ...
        set_admin(selected) {
            ...
            this.save(() => {
                ...
                let updates = {
                    ...
                    privilege: [
                        '我的考勤','考勤管理', '任务管理', '日报管理', '通讯目录',
                        '实时查岗', '工作轨迹', '费用报销', '绩效结算', '填写汇报',
                        '通知公告', '客户管理', '拜访管理', '扫名片', '用车申请',
                        '用章申请', '采购申请', '付款申请', '合同审批', '绩效报告',
                        '物品领用', '邀约到访', '销售业绩', '终端拜访', '铺货上报',
                        '库存上报', '销量上报', '综合上报', '终端形象采集', '竞品上报',
                        '订单管理', '商品管理', '库存查询', '销售订单', '销售退货单',
                        '收款登记', '客户应收', '调拨单据', '库存盘点', '车销',
                        '发货单', '进货单', '公司内部联络单', '员工评分'
                    ],
                ...
                axios.put('api/users', updates).then(resp => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `添加员工`})
                    }
                }).catch(error => {
                    console.error(error);
