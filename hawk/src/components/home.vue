            ...
            <f7-list-group v-for="(mod, i) in mods" :key="i" class="task-list-group">
                ...
                <f7-list-item class="task-list-layer task-list-content">
                    <f7-row
                            ...
                            v-if="index % 4 == 0"
                            style="display: flex; padding: 0 15px; justify-content: space-around;">

                        <f7-col class="task-container-col" v-for="n in [0,1,2,3]" :key="n">
...
.page-home {
    ...
    .task-container-col {
        ...
        display: flex;
        margin-bottom: 21px;
        width: calc((100% - 15px*2) / 4) !important;
    }
...
export default {
    ...
    watch: {
        myProfile: {
            handler () {
                if (this.myProfile && this.myProfile.bClientId) this.loadErpPermission()
            },
            immediate: true,
            deep: true
        }
    ...
    methods: {
        loadErpPermission () {
            axios.post('api/funPermission/funPermissionQuery', {
                bClientId: this.myProfile.bClientId
            }).then(res => {
        ...
        getModules() {
            ...
            let mods = JSON.parse(JSON.stringify(getValidMods(this.myProfile.role)));
            ...
            let myPermissions = [];
            let isCheckMyPermissions = false
            if (!this.superAdmin || (this.superAdmin && this.superAdmin._id != this.myUserId)) {
                myPermissions = this.userPermissions(this.myUserId)
                isCheckMyPermissions = true
            }

            mods = mods.filter(m => {
                let fData = this.funPermission.find(f => f.category === m.category)
                if (fData && (fData.val || fData.tasks.filter(f => f.val).length)) {
                    m.tasks = m.tasks.filter(t => {
                        return (
                            fData.tasks.find(v => v.name === t.name).val && (!isCheckMyPermissions || (isCheckMyPermissions && myPermissions.includes(t.name)))
                        )
                    });
