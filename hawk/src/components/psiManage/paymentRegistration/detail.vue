<template>
    <f7-page  @page:reinit="load">
        <f7-navbar :title="`${isTK ? '退款' : '收款'}单详情`" back-link>
            <f7-nav-right>
                <f7-link href="#" v-if="formData.status === '待审批' && formData.creator === myUserId" @click="selectPicker">操作</f7-link>
            </f7-nav-right>
        </f7-navbar>
        ...
        <div class="p_content">
            <div class="pc_layer">
                <div class="commodity_t">
                    <div class="title">
                        {{formData.name}}
                        <span :class="formData.status == '已审批' ? 'tip success' : formData.status == '被驳回' ? 'tip error' : 'tip'">{{formData.status}}</span>
                    </div>
                    <p>{{isTK ? '退款' : '收款'}}单编号：{{formData.coding}}</p>
                    <p>客户名称：{{formData.name}}</p>
                    <p>结算方式：{{formData.paymentType}}</p>
                    <p>{{isTK ? '退款' : '收款'}}金额：{{formData.receivable}}元</p>
                    <p>经办人：{{who(formData.operator) && who(formData.operator).name}}</p>
                    <p>提交人：{{who(formData.creator) && who(formData.creator).name}}</p>
                    <p>{{isTK ? '退款' : '收款'}}日期：{{moment(formData.time).format('YYYY-MM-DD HH:mm:ss')}}</p>
            ...
            <div class="footer" v-if="isShow">
                ...
                <div class="btn success" @click="agree(null, null)">同意</div>
            </div>
        </div>
        ...
        <f7-popup :opened="approver.show" @popup:closed="(approver.show = false)">
            <f7-page>
                ...
                <f7-block-title>通过审批，账单会进入财务申请流程，请选择</f7-block-title>
                <f7-list>
                    <f7-list-item title="财务人员" @click="open_personnelselector('lister')"
                                  :after="who(approver.lister) && who(approver.lister).name || '请选择'"></f7-list-item>
                    <!--<f7-list-item title="审核人" @click="open_personnelselector('reviewer')"-->
                                  <!--:after="who(approver.reviewer) && who(approver.reviewer).name || '请选择'"></f7-list-item>-->
                    <!--<f7-list-item title="审批人" @click="open_personnelselector('approver')"-->
                                  <!--:after="who(approver.approver) && who(approver.approver) .name || '请选择'"></f7-list-item>-->
                    <!--<f7-list-item title="出纳人" @click="open_personnelselector('bookkeeper')"-->
                                  <!--:after="who(approver.bookkeeper) && who(approver.bookkeeper).name || '请选择'"></f7-list-item>-->
                </f7-list>

                <f7-row style="padding: 0px 20px">
                    <f7-col>
                        <div class="submit_button" @click="approve">确定</div>
                    </f7-col>
                    <f7-col>
                        <div class="close_button" @click="approver.show=false">取消</div>
                    </f7-col>
                </f7-row>
...
export default {
    ...
    methods: {
        moment,
        load () {
            axios.get(`api/${this.isTK ? 'refundRegistration' : 'paymentRegistration'}/id/` + this.formData._id).then(res => {
                if (res.data) {
                    this.formData = res.data
                }
            }).catch(err => {
                console.error(err.message);
                $alert(`获取${this.isTK ? '退款' : '收款'}单详情失败`)
            })
        ...
        reject () {
            $prompt('请输入驳回原因', '驳回原因', (name) => {
                ...
                axios.post(`api/${this.isTK ? 'refundRegistration' : 'paymentRegistration'}`, data).then(res => {
                    $preloader.hide();
                    $alert('驳回成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回${this.isTK ? '退款' : '收款'}单`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('驳回失败！')
            ...
            $confirm(text || '确定批准该订单吗？', () => {
                ...
                if ((isDo && !text) || (!isDo && text)) {
                    ...
                    axios.post(`api/${this.isTK ? 'refundRegistration' : 'paymentRegistration'}`, data).then(res => {
                        ...
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `通过${this.isTK ? '退款' : '收款'}单`})
                        }
                    }).catch(err => {
                        $preloader.hide();
                        $alert('审批失败！')
        ...
        update () {
            this.$f7router.navigate(`/psiManage/paymentRegistration/update`, {
                props: {
                    uData: this.formData,
                    isTK: this.isTK
                }
        ...
        deleteFun () {
            $confirm(`确定删除该${this.isTK ? '退款' : '收款'}单吗？`, () => {
                $preloader.show();
                axios.delete(`api/${this.isTK ? 'refundRegistration' : 'paymentRegistration'}/`+ this.formData._id).then(res => {
                    $preloader.hide();
                    $alert('删除成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除${this.isTK ? '退款' : '收款'}单`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('删除失败！')
        ...
        approve() {
            if (this.approver.lister) {
                let finance = {
                    lister: this.approver.lister
                }
                this.agree('确定是否录入到财务', finance)
