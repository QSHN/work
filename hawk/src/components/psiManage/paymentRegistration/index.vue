<template>
    <f7-page @page:reinit="resetLoad">
        <f7-navbar title="收款记录" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                ...
                <div class="list_item" v-for="(v, i) in list" @click="$f7router.navigate('/psiManage/paymentRegistration/detail', { props: { detail: v }})">
                    <div class="title">
                        {{v.name}}
                    ...
                    <div class="other">
                        ...
                        <div class="name">{{who(v.operator) && who(v.operator).name}}</div>上传
                    </div>
                    <p>订单编号： {{v.coding}}</p>
                    <p>收款金额： {{v.receivable}}元</p>
                    <p>结算方式： {{v.paymentType}}</p>
                </div>
...
export default {
    ...
    methods: {
        ...
        searchFun () {
            this.$f7router.navigate('/psiManage/paymentRegistration/search', {
                props: {
                    search: (val) => {
                        this.filter = {...val}
        ...
        async load (state, cb) {
            ...
            await axios.post('api/paymentRegistration/list', this.filter).then(res => {
                this.identifier++;
