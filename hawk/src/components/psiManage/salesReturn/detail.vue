<template>
    <f7-page  @page:reinit="load">
        <f7-navbar title="退货单详情" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                <div class="commodity_t">
                    ...
                    <p>退货日期：{{moment(formData.time).format('YYYY-MM-DD HH:mm:ss')}}</p>
                    <p>退货仓库：{{formData.warehouse}}</p>
                ...
                <div class="commodity" v-if="formData.commodity.length">
                    ...
                    <div class="item" v-for="(v, i) in formData.commodity">
                        ...
                        <p>数量： {{v.number}} {{v.commodity.unit}}</p>
...
<script>
    ...
    export default {
        ...
        methods: {
            moment,
            load () {
                axios.get('api/salesReturn/id/' + this.formData._id).then(res => {
                    if (res.data) {
            ...
            reject () {
                $prompt('请输入驳回原因', '驳回原因', (name) => {
                    ...
                    axios.post('api/salesReturn', data).then(res => {
                        $preloader.hide();
                        $alert('驳回成功！', () => {
                            this.$f7router.back()
                        })
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回退货申请`})
                        }
                    }).catch(err => {
                        $preloader.hide();
                        $alert('驳回失败！')
            ...
            agree () {
                $confirm('确定批准该退货订单吗？', () => {
                    ...
                    axios.post('api/salesReturn', data).then(res => {
                        ...
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `通过退货申请`})
                        }
                    }).catch(err => {
                        $preloader.hide();
                        $alert('审批失败！')
            ...
            update () {
                this.$f7router.navigate('/psiManage/salesReturn/update', {
                    props: {
                        uData: this.formData
                    }
                })
            ...
            deleteFun () {
                $confirm('确定删除该退货单吗？', () => {
                    $preloader.show();
                    axios.delete('api/salesReturn/'+ this.formData._id).then(res => {
                        $preloader.hide();
                        $alert('删除成功！', () => {
                            this.$f7router.back()
                        })
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除退货申请`})
                        }
                    }).catch(err => {
                        $preloader.hide();
                        $alert('删除失败！')
