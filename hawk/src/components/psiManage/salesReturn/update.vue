<template>
    <f7-page>
        <f7-navbar title="退货上报" back-link></f7-navbar>
        ...
        methods: {
            ...
            submit () {
                if (!this.formData.name) {
                    ...
                    axios[this.uData ? 'post' : 'put']('api/salesReturn/', this.formData).then(res => {
                        $alert('退货上报成功', () => {
                            this.$f7router.back()
                        });
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `退货上报`})
                        }
                    }).catch(err => {
