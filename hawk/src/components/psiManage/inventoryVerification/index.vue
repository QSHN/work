<template>
    <f7-page @page:reinit="resetLoad">
        <f7-navbar title="库存盘点" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                ...
                <div class="list_item" v-for="(v, i) in list" @click="$f7router.navigate('/psiManage/inventoryVerification/detail', { props: { detail: v }})">
                    <div class="title">
                        {{v.coding}}
                        <span :class="v.status == '已入账' ? 'tip success' : v.status == '草稿' || v.status == '被驳回'  ? 'tip error' : 'tip'">{{v.status}}</span>
                    </div>
                    <div class="other">
                        <div class="time">{{moment(v.createdAt).format('YYYY-MM-DD HH:mm:ss')}}</div>
                        <div class="name">{{who(v.creator) && who(v.creator).name}}</div>上传
                    </div>
                    <p>盘点类型： {{v.inventoryType}}</p>
                    <p>盘点仓库： {{v.warehouse}}</p>
                </div>
...
export default {
    ...
    data () {
        return {
            status: '全部状态',
            statusList: ['全部状态', '草稿', '待入账', '已入账', '被驳回'],
            page: 0,
            isDo: true,
            identifier: 0,
            list: [],
    ...
    methods: {
        ...
        searchFun () {
            this.$f7router.navigate('/psiManage/inventoryVerification/search', {
                props: {
                    search: (val) => {
                        this.filter = {...val}
        ...
        async load (state, cb) {
            ...
            await axios.post('api/inventoryVerification/list', this.filter).then(res => {
                this.identifier++;
