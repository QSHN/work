<template>
    <f7-page>
        <f7-navbar title="新增库存盘点" back-link></f7-navbar>
        ...
        <div class="p_content">
            ...
            <div class="footer">
                <div class="btn" @click="submit(false)">保存至草稿</div>
                <div class="btn" @click="submit(true)">完成盘点</div>
            </div>
...
export default {
    ...
    data () {
        return {
            ...
            formData: {
                ...
                warehouse: '',
                operator: '',
                coding: `PD${Date.parse(new Date())}${Math.floor(Math.random () * 900) + 100}`,
                commodity: [],
                des: '',
                approver: [],
                status: '草稿',
    ...
    methods: {
        ...
        selectCommodity (defaultVal) {
            if (!this.formData.inventoryType || !this.formData.warehouse) {
                $alert('请先选择盘点类型和盘点仓库', () => {
            ...
            const picker = this.$f7.picker.create({
                ...
                on: {
                    change: (picker, values, displayValues) => {
                        self.formData.inventoryType = values[0]
                        // console.log(self.inventoryType)
                    }
                }
            });
            picker.open();
        ...
        submit (bool) {
            if (!this.formData.inventoryType) {
                ...
                this.formData.status = bool ? '待入账' : '草稿'
                $confirm(`是否确定${bool ? '完成盘点？' : '保存至草稿？'}`, () => {
                    axios[this.uData ? 'post' : 'put']('api/inventoryVerification/', this.formData).then(res => {
                        $alert(`库存盘点${this.uData ? '更新' : '新增'}成功`, () => {
                            this.$f7router.back();
                        });
                        if (bool) {
                            if (cordova.plugins.aMapLocation) {
                                this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '库存盘点'})
                            }
                        }
                    }).catch(err => {
                        console.error(err.message);
                        $alert(`库存盘点${this.uData ? '更新' : '新增'}失败`);
                    })
                })
            }
        }
    ...
    mounted() {
        if (this.uData) {
            ...
            this.formData.operator = this.myProfile._id
        }
    }
}
</script>
