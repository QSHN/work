<template>
    <f7-page @page:reinit="resetLoad">
        <f7-navbar title="订单列表" back-link>
            <f7-nav-right v-if="!uList">
        ...
        <div class="p_content">
            <div class="pc_layer">
                <div class="select_group" v-if="!uList">
                ...
                <div class="list_item" v-for="(v, i) in list" @click="$f7router.navigate('/psiManage/salesOrder/detail', { props: { detail: v }})">
                    <div class="title">
                        {{v.name}}
                    ...
                    <p>订单编号： {{v.coding}}</p>
                    <p>订单类型： {{v.coding.substr(0, 2) === 'XS' ? '销售订单' : '车销单'}}</p>
                    <p>订购品种： {{v.commodity.length}}</p>
                    <p>订购金额： {{v.allPrice}}</p>
                    <p>优惠金额： {{v.allPrice - v.receivable}}</p>
                    <p>应收金额： {{v.receivable}}</p>
                </div>
...
export default {
    ...
    methods: {
        ...
        async load (state, cb) {
            ...
            this.filter.creator = this.myUserId
            await axios.post('api/orderManage/list', this.filter).then(res => {
                this.identifier++;
