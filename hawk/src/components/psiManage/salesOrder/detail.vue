<template>
    <f7-page @page:reinit="load">
        <f7-navbar title="订单详情" back-link>
            <f7-nav-right>
...
export default {
    ...
    methods: {
            ...
            $prompt('请输入驳回原因', '驳回原因', (name) => {
                ...
                axios.post('api/salesOrder', data).then(res => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回销售订单`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('驳回失败！')
        ...
        agree () {
            $confirm('确定批准该订单吗？', () => {
                ...
                axios.post('api/salesOrder', data).then(res => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `通过销售订单`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('审批失败！')
        ...
        update () {
            this.$f7router.navigate('/psiManage/salesOrder/update', {
                props: {
                    uData: this.formData
                }
            })
        ...
        deleteFun () {
            $confirm('确定删除该销售订单吗？', () => {
                $preloader.show();
                axios.delete('api/salesOrder/'+ this.formData._id).then(res => {
                    $preloader.hide();
                    $alert('删除成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除销售订单`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('删除失败！')
