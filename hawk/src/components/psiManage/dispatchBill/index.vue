    ...
    <f7-page @page:reinit="resetLoad">
        ...
        <div class="p_content">
            <div class="pc_layer">
                ...
                <div class="list_item" v-for="(v, i) in list" @click="$f7router.navigate('/psiManage/dispatchBill/detail', { props: { detail: v }})">
                    <div class="title">
                        {{v.name}}
                        <span :class="v.shippingStatus ? 'tip success' :  'tip'">{{v.shippingStatus ? '已发货' : '待发货'}}</span>
                    </div>
                    <div class="other">
                        <div class="time">{{moment(v.createdAt).format('YYYY-MM-DD HH:mm:ss')}}</div>
                        <div class="name">{{who(v.creator) && who(v.creator).name}}</div>上传
                    </div>
                    <p>发货仓库： {{v.warehouse}}</p>
                    <p>交货日期： {{moment(v.time).format('YYYY-MM-DD HH:mm:ss')}}</p>
                </div>
...
export default {
    ...
    watch: {
        status () {
            this.filter.shippingStatus = this.status === '已发货'
            this.resetLoad()
        }
    },
    methods: {
        moment,
        addFun () {
            this.$f7router.navigate('/psiManage/dispatchBill/update')
        },
        searchFun () {
            this.$f7router.navigate('/psiManage/dispatchBill/search', {
                props: {
                    search: (val) => {
                        this.filter = {...val}
                        ...
                        if (val.shippingStartTime) this.filter.shippingStartTime = new Date(val.shippingStartTime).toHawkDateString() + ' 00:00:00.000'
                        if (val.shippingEndTime) this.filter.shippingEndTime = new Date(val.shippingEndTime).toHawkDateString() + ' 23:59:59.999'
                        this.filter.page = 1
        ...
        async load (state, cb) {
            ...
            await axios.post('api/salesOrder/list', this.filter).then(res => {
                this.identifier++;
