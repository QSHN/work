<template>
    <f7-page  @page:reinit="load">
        <f7-navbar title="回库申请详情" back-link>
            <f7-nav-right>
            ...
            <div class="pc_layer">
                ...
                <div class="commodity" v-if="formData.commodity.length">
                    ...
                    <div class="item" v-for="(v, i) in formData.commodity">
                        ...
                        <p>{{formData.backType === 'all' ? '全部' : formData.backType === 'return' ? '退货' : formData.backType === 'swap' ? '换货' : '部分'}}归还： {{v.number}} {{v.commodity.unit}}</p>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        moment,
        load () {
            axios.get('api/carSales/back/id/' + this.formData._id).then(res => {
                if (res.data) {
        ...
        reject () {
            $prompt('请输入驳回原因', '驳回原因', (name) => {
                ...
                axios.post('api/carSales/back', data).then(res => {
                    $preloader.hide();
                    $alert('驳回成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '驳回回库申请'})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('驳回失败！')
        ...
        agree () {
            $confirm('确定批准该回库申请吗？', () => {
                ...
                axios.post('api/carSales/back', data).then(res => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '审批回库申请'})
                    }
                }).catch(err => {
                    $preloader.hide();
        ...
        update () {
            this.$f7router.navigate('/psiManage/carSales/back/update', {
                props: {
                    uData: this.formData,
                    backType: this.formData.backType,
                    backName: this.getName(this.formData.backType)
                }
            })
        },
        deleteFun () {
            $confirm('确定删除该回库申请吗？', () => {
                $preloader.show();
                axios.delete('api/carSales/back/'+ this.formData._id).then(res => {
                    $preloader.hide();
                    $alert('删除成功！', () => {
                        this.$f7router.back()
                    })
