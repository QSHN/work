        ...
        methods: {
            ...
            submit () {
                if (!this.formData.warehouse) {
                    ...
                    axios[this.uData ? 'post' : 'put']('api/carSales/bill/', this.formData).then(res => {
                        $alert('提交提货申请成功', () => {
                            this.$f7router.back()
                        });
                        if (!this.uData) {
                            if (cordova.plugins.aMapLocation) {
                                this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '提货申请'})
                            }
                        }
                    }).catch(err => {
