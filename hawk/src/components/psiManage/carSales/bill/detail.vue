<template>
    <f7-page  @page:reinit="load">
        <f7-navbar title="提货申请详情" back-link>
            <f7-nav-right>
            ...
            <div class="pc_layer">
                ...
                <div class="commodity" v-if="formData.commodity.length">
                    ...
                    <div class="item" v-for="(v, i) in formData.commodity">
                        ...
                        <p>提货数量： {{v.number}} {{v.commodity.unit}}</p>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        moment,
        load () {
            axios.get('api/carSales/bill/id/' + this.formData._id).then(res => {
                if (res.data) {
        ...
        reject () {
            $prompt('请输入驳回原因', '驳回原因', (name) => {
                ...
                axios.post('api/carSales/bill', data).then(res => {
                    $preloader.hide();
                    $alert('驳回成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回提货申请`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('驳回失败！')
        ...
        agree () {
            $confirm('确定批准该提货申请吗？', () => {
                ...
                axios.post('api/carSales/bill', data).then(res => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `通过提货申请`})
                    }
                }).catch(err => {
                    $preloader.hide();
        ...
        update () {
            this.$f7router.navigate('/psiManage/carSales/bill/update', {
                props: {
                    uData: this.formData
                }
            })
        ...
        deleteFun () {
            $confirm('确定删除该提货申请吗？', () => {
                $preloader.show();
                axios.delete('api/carSales/bill/'+ this.formData._id).then(res => {
                    $preloader.hide();
                    $alert('删除成功！', () => {
                        this.$f7router.back()
                    })
