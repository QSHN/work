<template>
    <f7-page @page:reinit="resetLoad">
        <f7-navbar title="提货申请记录" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                ...
                <div class="list_item" v-for="(v, i) in list" @click="$f7router.navigate('/psiManage/carSales/bill/detail', { props: { detail: v }})">
                    <div class="title">
                        {{v.numberPlate}}
...
export default {
    ...
    methods: {
        ...
        searchFun () {
            this.$f7router.navigate('/psiManage/carSales/bill/search', {
                props: {
                    search: (val) => {
                        this.filter = {...val}
        ...
        async load (state, cb) {
            ...
            await axios.post('api/carSales/bill/list', this.filter).then(res => {
                this.identifier++;
