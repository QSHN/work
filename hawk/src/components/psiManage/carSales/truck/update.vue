<template>
    <f7-page>
        <f7-navbar title="装车拍照" back-link></f7-navbar>
    ...
    methods: {
        ...
        submit () {
            if (!this.formData.numberPlate) {
                ...
                axios[this.uData ? 'post' : 'put']('api/carSales/truck/', this.formData).then(res => {
                    $alert('装车拍照上传成功', () => {
                       this.$f7router.back();
                    });
                    if (!this.uData) {
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '装车拍照'})
                        }
                    }
                }).catch(err => {
        ...
        if (this.uData) {
            ...
            axios.post('api/carSales/car/list', {
                ...
                if (res.data && res.data.length) {
                    this.formData.numberPlate = res.data[0].numberPlate
                }
            }).catch(err => {
                console.error(err.message);
                $alert('获取不到您的提货车辆')
