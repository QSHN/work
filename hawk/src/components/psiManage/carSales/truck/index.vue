<template>
    <f7-page @page:reinit="resetLoad">
        <f7-navbar title="装车拍照记录" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                <div class="list_item" v-for="(v, i) in list" @click="$f7router.navigate('/psiManage/carSales/truck/detail', { props: { detail: v }})">
                    <div class="list_l">
                        <div class="title">
                            {{v.numberPlate}}
                        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            searchFun () {
                this.$f7router.navigate('/psiManage/carSales/truck/search', {
                    props: {
                        search: (val) => {
                            this.filter = {...val}
            ...
            async load (state, cb) {
                ...
                await axios.post('api/carSales/truck/list', this.filter).then(res => {
                    this.identifier++;
