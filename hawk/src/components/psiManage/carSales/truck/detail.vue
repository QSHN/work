<template>
    <f7-page class="image_acquisition"  @page:reinit="load">
        <f7-navbar title="装车拍照详情" back-link>
            <f7-nav-right>
                ...
                <div class="list-item clear-float">
                    ...
                    <div style="float: right">
                        <input class="item-input"
                               type="text"
                               placeholder="请创建"
                               :value="who(formData.creator) && who(formData.creator).name"
                               style="padding-right: 5px;"
                               readonly />
                    </div>
                </div>
                ...
                <div class="list-item clear-float">
                    ...
                    <div style="float: right">
                        <input class="item-input"
                               type="text"
                               placeholder="请创建"
                               :value="moment(formData.createdAt).format('YYYY-MM-DD HH:mm:ss')"
                               style="padding-right: 5px;"
                               readonly />
                    </div>
                </div>
...
export default {
    ...
    methods: {
        moment,
        load () {
            axios.get('api/carSales/truck/id/' + this.formData._id).then(res => {
                if (res.data) {
        ...
        update () {
            this.$f7router.navigate('/psiManage/carSales/truck/update', {
                props: {
                    uData: this.formData
                }
            })
        ...
        deleteFun () {
            $confirm('确定删除该装车拍照吗？', () => {
                $preloader.show();
                axios.delete('api/carSales/truck/'+ this.formData._id).then(res => {
                    $preloader.hide();
                    $alert('删除成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除装车拍照`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('删除失败！')
