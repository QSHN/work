<template>
    <f7-page  @page:reinit="load">
        <f7-navbar title="车载库存盘点详情" back-link>
            <f7-nav-right>
            ...
            <div class="pc_layer">
                ...
                <div class="commodity" v-if="formData.commodity.length">
                    ...
                    <div class="item" v-for="(v, i) in formData.commodity">
                        ...
                        <p>账面数量： {{v.realNumber}} {{v.commodity.unit}}</p>
                        <p>实盘数量： {{v.number}} {{v.commodity.unit}}</p>
...
export default {
    ...
    methods: {
        moment,
        load () {
            axios.get('api/carSales/inventoryCheck/id/' + this.formData._id).then(res => {
                if (res.data) {
        ...
        reject () {
            $prompt('请输入驳回原因', '驳回原因', (name) => {
                ...
                axios.post('api/carSales/inventoryCheck', data).then(res => {
                    $preloader.hide();
                    $alert('驳回成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回车载盘点`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('驳回失败！')
        ...
        agree () {
            $confirm('确定批准该车载库存盘点吗？', () => {
                ...
                axios.post('api/carSales/inventoryCheck', data).then(res => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `通过车载盘点`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('审批失败！')
        ...
        update () {
            this.$f7router.navigate('/psiManage/carSales/inventoryCheck/update', {
                props: {
                    uData: this.formData
                }
            })
        ...
        deleteFun () {
            $confirm('确定删除该车载库存盘点吗？', () => {
                $preloader.show();
                axios.delete('api/carSales/inventoryCheck/'+ this.formData._id).then(res => {
                    $preloader.hide();
                    $alert('删除成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除库存盘点`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('删除失败！')
