<template>
    <f7-page>
        <f7-navbar title="车载库存盘点" back-link></f7-navbar>
        ...
        methods: {
            ...
            submit () {
                if (!this.formData.numberPlate) {
                    ...
                    axios[this.uData ? 'post' : 'put']('api/carSales/inventoryCheck/', this.formData).then(res => {
                        $alert('车载库存盘点上报成功', () => {
                            this.$f7router.back()
                        });
                        if (!this.uData) {
                            if (cordova.plugins.aMapLocation) {
                                this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '车载盘点'})
                            }
                        }
                    }).catch(err => {
