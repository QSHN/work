        ...
        <div class="p_content">
            ...
            <div class="footer">
                <div class="price">
                    ...
                    <!--<p>-->
                        <!--销售： <span>{{getAllPrice('salesPrice')}}元</span>-->
                        <!--退货： <span>{{getAllPrice('returnPrice')}}元</span>-->
                    <!--</p>-->
                </div>
...
<script>
    ...
    export default {
        ...
        data () {
            return {
                ...
                formData: {
                    ...
                    approver: [],
                    status: '待审批',
                    coding: `CXD${Date.parse(new Date())}${Math.floor(Math.random () * 900) + 100}`
                },
                inventory: [],
        ...
        computed: {
            ...
            getAllPrice () {
                this.formData.allPrice = this.formData.commodity.reduce((all, v) => all += v.salesNumber * v.salesPrice, 0)
        ...
        methods: {
            ...
            submit () {
                if (!this.formData.numberPlate) {
                    ...
                    axios[this.uData ? 'post' : 'put']('api/carSales/order/', this.formData).then(res => {
                        $alert('车销单生成成功', () => {
                            this.$f7router.back()
                        });
                        if (!this.uData) {
                            if (cordova.plugins.aMapLocation) {
                                this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '车销单'})
                            }
                        }
                    }).catch(err => {
