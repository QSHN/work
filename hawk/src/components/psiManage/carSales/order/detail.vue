<template>
    <f7-page  @page:reinit="load">
        <f7-navbar title="车销单详情" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                <div class="commodity_t">
                    ...
                    <p>车销单编号：{{formData.coding}}</p>
                    <p>车&nbsp;&nbsp;&nbsp;&nbsp;号：{{formData.numberPlate}}</p>
                ...
                <div class="commodity" v-if="formData.commodity.length">
                    ...
                    <div class="item" v-for="(v, i) in formData.commodity">
                        ...
                        <p>销量单价： {{v.salesPrice}}元 / {{v.commodity.unit}}</p>
                        <p>销售数量： {{v.salesNumber}} {{v.commodity.unit}}</p>
                        <p>销售金额： {{v.salesPrice * v.salesNumber}} 元</p>
                        <p class="line">销售备注： {{v.salesDes || '无'}}</p>
                        <p>赠品数量： {{v.giveNumber}} {{v.commodity.unit}}</p>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        moment,
        load () {
            axios.get('api/carSales/order/id/' + this.formData._id).then(res => {
                if (res.data) {
        ...
        reject () {
            $prompt('请输入驳回原因', '驳回原因', (name) => {
                ...
                axios.post('api/carSales/order', data).then(res => {
                    $preloader.hide();
                    $alert('驳回成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回车销单`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('驳回失败！')
            ...
            $confirm('确定批准该车销单吗？', () => {
                ...
                if ((isDo && !text) || (!isDo && text)) {
                    ...
                    axios.post('api/carSales/order', data).then(res => {
                        ...
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `审批通过车销单`})
                        }
                    }).catch(err => {
                        $preloader.hide();
        ...
        update () {
            this.$f7router.navigate('/psiManage/carSales/order/update', {
                props: {
                    uData: this.formData
                }
            })
        ...
        deleteFun () {
            $confirm('确定删除该车销单吗？', () => {
                $preloader.show();
                axios.delete('api/carSales/order/'+ this.formData._id).then(res => {
                    $preloader.hide();
                    $alert('删除成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除车销单`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('删除失败！')
