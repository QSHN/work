<template>
    <f7-page  @page:reinit="load">
        <f7-navbar title="退换货详情" back-link>
            <f7-nav-right>
...
export default {
    ...
    methods: {
        moment,
        load () {
            axios.get('api/carSales/refundable/id/' + this.formData._id).then(res => {
                if (res.data) {
        ...
        reject () {
            $prompt('请输入驳回原因', '驳回原因', (name) => {
                ...
                axios.post('api/carSales/refundable', data).then(res => {
                    $preloader.hide();
                    $alert('驳回成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回退换货申请`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('驳回失败！')
            ...
            $confirm('确定批准该退换货单吗？', () => {
                ...
                if ((isDo && !text) || (!isDo && text)) {
                    ...
                    axios.post('api/carSales/refundable', data).then(res => {
                        ...
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `审批通过退换货`})
                        }
                    }).catch(err => {
                        $preloader.hide();
                        $alert('审批失败！')
        ...
        deleteFun () {
            $confirm('确定删除该车销单吗？', () => {
                $preloader.show();
                axios.delete('api/carSales/refundable/'+ this.formData._id).then(res => {
                    $preloader.hide();
                    $alert('删除成功！', () => {
                        this.$f7router.back()
                    })
