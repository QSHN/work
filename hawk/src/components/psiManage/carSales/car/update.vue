<template>
    <f7-page>
        <f7-navbar title="新增车辆" back-link></f7-navbar>
        ...
        <div class="p_content">
            <div class="pc_layer">
                ...
                <div class="list-item clear-float" @click="selectSalesman">
                    <span class="item-icon">*</span>
                    <label>跟车员业务员：</label>
                    <div style="float: right">
                ...
                <div class="list-item clear-float" @click="select">
                    <span class="item-icon"></span>
                    <label>状态：</label>
                    <div style="float: right">
                        <div style="width: 20px; height: 46px; float: right;">
...
<script>
    ...
    export default {
        ...
        props: ['uData', 'isMyCreated', 'update'],
        data () {
            return {
                formData: {
                    status: '启用',
        ...
        methods: {
            select () {
                ...
                const picker = this.$f7.picker.create({
                    ...
                    cols: [
                        {
                            textAlign: 'center',
                            values: Array.from(self.status),
                        }
                    ],
                    on: {
                        change: (picker, values, displayValues) => {
                            self.formData.status = values[0]
                        }
                    }
                });
                picker.open();
            ...
            selectSalesman () {
              if (!this.isMyCreated) this.selectorOperatorShow = true
            },
            onSelectorPersonne(uIds) {
                this.selectorOperatorShow = false;
                this.formData.salesman = uIds[0].toString();
            },
            submit () {
                if (!this.formData.numberPlate) {
                    $alert('请输入车号')
                } else if (!this.formData.salesman) {
                    $alert('请选择跟车业务员')
                } else {
                    axios[this.uData ? 'post' : 'put'](`api/carSales/car`, this.formData).then(res => {
                        $alert(`${this.uData ? '更新' : '新建'}车辆成功`, () => {
                            if (this.isMyCreated) this.update(this.formData.numberPlate)
                            this.$f7router.back()
                        });
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `${this.uData ? '更新' : '新建'}车辆`})
                        }
                    }).catch(err => {
                        console.error(err.message);
                        $alert(`${this.uData ? '更新' : '新建'}车辆失败,请检查是否存在该车号或跟车业务员`);
                    })
                }
            }
        },
        mounted() {
            ...
            if (this.isMyCreated) this.formData.salesman = this.myUserId
        }
    }
</script>
