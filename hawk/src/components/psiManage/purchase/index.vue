    ...
    <f7-page @page:reinit="resetLoad">
        ...
        <div class="p_content">
            <div class="pc_layer">
                ...
                <div class="list_item" v-for="(v, i) in list" @click="$f7router.navigate('/psiManage/purchase/detail', { props: { detail: v }})">
                    <div class="title">
                        {{v.coding}}
                    ...
                    <p>进货仓库： {{v.warehouse}}</p>
                    <p>进货品种： {{v.commodity.length}}</p>
                    <p>进货金额： {{v.allPrice}}元</p>
                    <p>备注：{{v.des || '无'}}</p>
                </div>
...
export default {
    ...
    methods: {
        ...
        searchFun () {
            this.$f7router.navigate('/psiManage/purchase/search', {
                props: {
                    search: (val) => {
                        this.filter = {...val}
        ...
        async load (state, cb) {
            ...
            await axios.post('api/purchase/list', this.filter).then(res => {
                this.identifier++;
