<template>
    <f7-page  @page:reinit="load">
        <f7-navbar title="进货单详情" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                <div class="commodity_t">
                    ...
                    <p>进货仓库：{{formData.warehouse}}</p>
                    <p>经办人：{{who(formData.creator) && who(formData.creator).name}}</p>
                ...
                <div class="commodity" v-if="formData.commodity.length">
                    ...
                    <div class="item" v-for="(v, i) in formData.commodity">
                        ...
                        <p>进货金额： {{v.price * v.number}}元</p>
                    </div>

                    <div class="all_price">
                        <span>合计金额</span>
                        <p>{{formData.allPrice}}元</p>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        moment,
        load () {
            axios.get('api/purchase/id/' + this.formData._id).then(res => {
                if (res.data) {
        ...
        reject () {
            $prompt('请输入驳回原因', '驳回原因', (name) => {
                ...
                axios.post('api/purchase', data).then(res => {
                    $preloader.hide();
                    $alert('驳回成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回进货申请`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('驳回失败！')
            ...
            $confirm(text || '确定通过该进货单吗？', () => {
                ...
                if ((isDo && !text) || (!isDo && text)) {
                    ...
                    axios.post(`api/purchase`, data).then(res => {
                        ...
                        if (cordova.plugins.aMapLocation) {
                            this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `通过进货申请`})
                        }
                    }).catch(err => {
                        $preloader.hide();
                        $alert('审批失败！')
        ...
        update () {
            this.$f7router.navigate('/psiManage/purchase/update', {
                props: {
                    uData: this.formData
                }
            })
        ...
        deleteFun () {
            $confirm('确定删除该进货单吗？', () => {
                $preloader.show();
                axios.delete('api/purchase/'+ this.formData._id).then(res => {
                    $preloader.hide();
                    $alert('删除成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除进货单`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('删除失败！')
