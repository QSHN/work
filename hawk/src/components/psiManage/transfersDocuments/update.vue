<template>
    <f7-page>
        <f7-navbar title="新增调拨单据" back-link></f7-navbar>

        <div class="p_content">
            <div class="pc_layer">
                <div class="list-item clear-float" @click="selectClient('warehouseOut')">
                    ...
                    <div style="float: right">
                        ...
                        <input class="item-input"
                               type="text"
                               placeholder="请选择"
                               v-model="formData.warehouseOut"
                               style="padding-right: 5px;"
                               readonly />
                    </div>
                </div>

                <div class="list-item clear-float" @click="selectClient('warehouseIn')">
                    <span class="item-icon">*</span>
                    <label>调入仓库：</label>
                    <div style="float: right">
                        <div style="width: 20px; height: 46px; float: right;">
...
export default {
    ...
    data () {
        return {
            ...
            formData: {
                warehouseOut: '',
                warehouseIn: '',
                coding: `DB${Date.parse(new Date())}${Math.floor(Math.random () * 900) + 100}`,
                commodity: [],
                des: '',
                approver: [],
                status: '待审批',
    ...
    methods: {
        ...
        selectClient (which) {
            this.$f7router.navigate('/selectList', {
                props: {
                    ...
                    url: 'warehouse/',
                    choose: (type, val) => {
                        this.formData[which] = val.name
                        if (which === 'warehouseOut') this.warehouse = val
                        this.$f7router.back()
                    }
                }
            })
        ...
        selectCommodity (defaultVal) {
            if (!this.formData.warehouseOut) {
                $alert('请先选择调出仓库', () => {
                    this.selectClient('warehouseOut')
                })
            } if (this.formData.warehouseOut === this.formData.warehouseIn) {
                $alert('调出仓库不能和调入仓库一样', () => {
                    this.selectClient('warehouseIn')
                })
            } else {
                this.$f7router.navigate('/selectCommodity', {
                    props: {
                        ...
                        inventoryType: this.formData.inventoryType,
        ...
        submit (bool) {
            if (!this.formData.warehouseOut) {
                $alert('请选择调出仓库')
            } else if (!this.formData.warehouseIn) {
                $alert('请选择调入仓库')
                ...
                axios[this.uData ? 'post' : 'put']('api/transfersDocuments/', this.formData).then(res => {
                    $alert(`调拨单据${this.uData ? '更新' : '新增'}成功`, () => {
                       this.$f7router.back();
                    });
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `调拨单`})
                    }
                }).catch(err => {
                    console.error(err.message);
                    $alert(`调拨单据${this.uData ? '更新' : '新增'}失败`);
                })
            }
        }
    },
