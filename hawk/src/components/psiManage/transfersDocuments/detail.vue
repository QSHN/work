<template>
    <f7-page  @page:reinit="load">
        <f7-navbar title="调拨单详情" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                <div class="commodity_t">
                    ...
                    <p>调出仓库：{{formData.warehouseOut}}</p>
                    <p>调入仓库：{{formData.warehouseIn}}</p>
...
export default {
    ...
    methods: {
        moment,
        load () {
            axios.get('api/transfersDocuments/id/' + this.formData._id).then(res => {
                if (res.data) {
        ...
        reject () {
            $prompt('请输入驳回原因', '驳回原因', (name) => {
                ...
                axios.post('api/transfersDocuments', data).then(res => {
                    $preloader.hide();
                    $alert('驳回成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `驳回调拨单`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('驳回失败！')
        ...
        agree () {
            $confirm('确定通过该调拨单据吗？', () => {
                ...
                axios.post('api/transfersDocuments', data).then(res => {
                    ...
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `通过调拨单`})
                    }
                }).catch(err => {
                    $preloader.hide();
        ...
        update () {
            this.$f7router.navigate('/psiManage/transfersDocuments/update', {
                props: {
                    uData: this.formData
                }
            })
        ...
        deleteFun () {
            $confirm('确定删除该调拨单据吗？', () => {
                $preloader.show();
                axios.delete('api/transfersDocuments/'+ this.formData._id).then(res => {
                    $preloader.hide();
                    $alert('删除成功！', () => {
                        this.$f7router.back()
                    })
                    if (cordova.plugins.aMapLocation) {
                        this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: `删除调拨单据`})
                    }
                }).catch(err => {
                    $preloader.hide();
                    $alert('删除失败！')
