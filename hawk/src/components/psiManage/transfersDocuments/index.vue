<template>
    <f7-page @page:reinit="resetLoad">
        <f7-navbar title="调拨单据" back-link>
            <f7-nav-right>
        ...
        <div class="p_content">
            <div class="pc_layer">
                ...
                <div class="list_item" v-for="(v, i) in list" @click="$f7router.navigate('/psiManage/transfersDocuments/detail', { props: { detail: v }})">
                    <div class="title">
                        {{v.coding}}
                    ...
                    <p>调出仓库： {{v.warehouseOut}}</p>
                    <p>调入仓库： {{v.warehouseIn}}</p>
...
export default {
    ...
    methods: {
        ...
        searchFun () {
            this.$f7router.navigate('/psiManage/transfersDocuments/search', {
                props: {
                    search: (val) => {
                        this.filter = {...val}
        ...
        async load (state, cb) {
            ...
            await axios.post('api/transfersDocuments/list', this.filter).then(res => {
                this.identifier++;
