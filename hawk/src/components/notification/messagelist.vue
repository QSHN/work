          ...
          <div class="message-list">
              <div v-if="msgList.length>0" class="message-item" v-for="(item,i) in msgList" @click="goDetail(i)">
                  ...
                  <div class="msg">
                      ...
                      <div class="title" :style="{color: item.status === '未读' ? '#3385FF' : '#999'}">
                          {{item.title}}
                          <!--<div class="color-box" v-if="item.status == '未读'"  ></div>-->
                      </div>
                      <div class="content">{{ chearBr(item.content) }}</div>
                  </div>
        ...
        delete(){
            ...
            axios.delete("/api/notification/removeall/"+this.myProfile._id).then(resp=>{
                ...
                if (cordova.plugins.aMapLocation) {
                    this.$store.dispatch('createMarker', {device: device, aMapLocation: cordova.plugins.aMapLocation, content: '清空消息列表'})
                }
            }).catch(err => {
                $preloader.hide();
                console.error(err);
