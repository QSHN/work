      ...
      <template v-for="(v, i) in tempList">
        ...
        >
          <template slot-scope="scope">
            ...
            <template v-else>
              <div v-html="getValue(scope.row, v.code, v.type)" />
            </template>
          </template>
        </el-table-column>
      </template>
...
export default {
  ...
  computed: {
    ...
    tempList() {
      if (!this.loading && this.tempData) {
        return this.tempData.temp[this.whichType === '合同' ? 'purchaseContractDetailTable' : 'purchaseOrderDetailTable'].temp
      } else {
        return []
      }
    }
  ...
  methods: {
    ...
    load(clear, currentPage) {
      ...
      const isSub = this.judgeAuthority('采购明细列表', 1, '查看自己和下属的采购订单和合同')
      const isAll = this.judgeAuthority('采购明细列表', 1, '查看所有采购订单和合同')
      let subList = []
      if (isSub) {
        ...
        if (c[0].includes('Table')) {
          if (row.temp[c[0]]) {
            return row.temp[c[0]].reduce((total, r, i) => {
              if (type === 'number') {
                total += `<div>${this.formatMoney(r[c[1]] || 0, '', 2)}</div>`
              } else {
                total += `<div>${r[c[1]]}</div>`
              }
              return total
    ...
    setTemp(row, whichTemp, cb) {
      this.$refs.createTemp.show(row, whichTemp, this.tempData.temp, this.whichType === '合同' ? 'purchaseContractDetailTable' : 'purchaseOrderDetailTable', cb)
    },
    custom() {
      if (!this.judgeAuthority('采购明细列表', 0, '自定义')) {
        this.$message.error(`对不起，您没有自定义采购明细列表的权限，请联系管理员`)
        return
      }
      this.$refs.temp.show(this.tempData, this.whichType === '合同' ? 'purchaseContractDetailTable' : 'purchaseOrderDetailTable')
    },
    exportFun() {
      if (!this.judgeAuthority('采购明细列表', 0, '导出')) {
        this.$message.error(`对不起，您没有导出采购明细列表的权限，请联系管理员`)
        return
      }
      ...
      exportData({
        ...
        const fileName = `采购明细表${this.moment().format('YYYY-MM-DD')}.xls`
