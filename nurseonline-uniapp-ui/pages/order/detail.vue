<template>
	<view>
		<view class="padding-20 justify-between detail-top bg-white bottom-20">
			<view class="">
				<view class="">单号：<text class="font-blue">{{bill.billId}}</text></view>
			...
			<view class="flex-column justify-between">
				<text class="font-red"><text class="iconfont">&#xe835;</text>{{billStates[bill.billState]}}</text>
				<!-- <view class="button-area">
					<button class="grey" v-if="bill.billState == 0">撤销订单</button>
					<button v-else-if="bill.billState == 1">查看预约</button>
				</view> -->
				<template v-if="bill.billState !== 6">
					...
					<view class="button-area">
						...
						<template v-else>
							<button v-if="bill.billState === 3" @tap="billConfirm">确认订单</button>
							<template v-if="bill.billState === 5">
								<button @tap="toRate" v-if="bill.reviewFlag === -1">订单评价</button>
								<button @tap="toRate" v-else-if="bill.reviewFlag > 0">查看评价</button>
							</template>
							<button @tap="appoint" v-if="bill.billState >= 2">查看预约</button>
							<!-- <template v-if="bill.billState === 2">
								<button>提交预约</button>
								<button @tap="appoint">查看预约</button>
							</template>
							<template v-else>
								<template v-if="bill.billState === 3">
									<button>确认订单</button>
									<button @tap="appoint">查看预约</button>
								</template>
								<template v-else-if="bill.billState === 4">
									<button @tap="appoint">查看预约</button>
								</template>
								<template v-else-if="bill.billState === 5">
									<button @tap="toRate">订单评价</button>
									<button @tap="appoint">查看预约</button>
								</template>
							</template> -->
						</template>
			...
			<title-card title="支付信息">
				<template slot="content">
					<view class="padding">
						...
						<view class="bottom-20">支付状态：{{bill.payState == 0 ? '未支付' : '已支付'}}</view>
						<template v-if="bill.payState == 0 && bill.billState < 2">
			...
			<title-card title="服务对象信息">
				<template slot="content">
					<view class="padding">
						...
						<view class="">服务地址：{{bill.custAddress}}</view>
				...
				<template slot="content">
					<view class="padding">
						<!-- <view class="bottom-20">
							<view>{{bill.serviceLabel}}</view>
							<view>服务费：¥{{bill.price | priceFormat}} x {{bill.qty}}{{bill.unit}} = ¥{{bill.price * bill.qty | priceFormat}}</view>
						</view>
						<view class="bottom-20" v-if="bill.kitGoodsLabel">
							<view>{{bill.kitGoodsLabel}}</view>
							<view>耗材费：¥{{bill.kitPrice | priceFormat}} x {{bill.kitScale}} x {{bill.qty}}{{bill.unit}} = ¥{{bill.kitPrice * bill.kitScale * bill.qty | priceFormat}}</view>
						</view>
						<view class="bottom-20">
							<view>上门费：¥{{bill.visitPrice | priceFormat}} x {{bill.qty}}{{bill.unit}} = ¥{{bill.visitPrice * bill.qty | priceFormat}}</view>
						</view>
						<view class="bottom-20">
							<view>保险费：¥{{bill.premiumAmt | priceFormat}}</view>
						</view> -->
						<!-- <view class="bottom-20">
						...
						<view class="bottom-20 tl">
							<view>{{bill.serviceLabel}}：¥{{priceAdjust}}x{{bill.qty}}次</view>
							<!-- <view>保险费：¥{{bill.discountAmt | priceFormat}}</view> -->
							<!-- <view>优惠金额：¥{{bill.discountAmt | priceFormat}}</view> -->
							<view>订单金额：¥{{bill.paymentAmt + bill.couponAmt | priceFormat}}</view>
							<!-- <view>已付金额（礼券金额）：¥{{bill.couponAmt | priceFormat}}</view> -->
...
export default {
	data() {
		return {
			bill: {},
			billStates: ['草稿','已提交','已审核（待服务）','已服务（待确认）','（会员）已确认','交易成功关闭','交易撤销关闭'],
			billId: '',
			billCerts: [],
			countTime: 600,
			minutes: '',
	...
	methods: {
		fetchData() {
			getBillDetail(this.billId).then(({data}) => {
				let res_data;
				if(data.msg === 'success'){
		...
		billCancel() {
			uni.showModal({
				...
				success: res => {
					if(res.confirm){
						cancelBill(this.bill.billId).then(response => {
							let data = response.data
							if(data.msg === 'success'){
								uni.showToast({
									title: '订单取消成功！',
		...
		billConfirm(){
			uni.showModal({
				...
				success: res => {
					if(res.confirm){
						confirmBill(this.bill.billId).then(response => {
							let data = response.data
							if(data.msg === 'success'){
								uni.showToast({
									title: '订单确认成功！',
