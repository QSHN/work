<template>
	<view class="mask" @tap="tapMask" :class="!value?'':'mask-show'">
		...
		<block v-if="type=='input'" @touchmove>
		...
		<block v-if="type == 'rich-text'">
			<view class="rich-view" @tap.stop>
				<view class="title">{{mData.title}}</view>
				<view class="content word-break"><slot></slot></view>
				<view class="button-area pb20">
					<button @tap="tapCancel">我知道了</button>
				</view>
			</view>
		</block>
	</view>
			...
			value:{
				...
				handler(newVal,oldVal){
					...
					if(newVal) {
						...
						page.classList.add('popup-parent--hidden')
						this.showPlusMask(0.6)
					}else{
						page.classList.remove('popup-parent--hidden')
