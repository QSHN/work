...
const WarehouseDiskDeficient = require('./warehouseDiskDeficient.model');
const Warehouse = require('../warehouse/warehouse.model');
const NP = require('number-precision');

module.exports = {
    updateWarehouseDiskDeficient,
    queryWarehouseDiskDeficient,
    batchWarehouseDiskDeficient,
    parameterWarehouseDiskDeficient
};

async function updateWarehouseDiskDeficient(req, res, next) {
    L('Calling API WarehouseDiskDeficient.updateWarehouseDiskDeficient', req.body);
    ...
    try {
        ...
        if (!data.accountSetId) {
            ...
            delete data._id
            if (_id) {
                await WarehouseDiskDeficient.updateOne({_id}, { $set: data })
            } else {
                let result = await WarehouseDiskDeficient.findOne({
                    code: data.code
                })
                if (result) {
                    error = '已存在该单据编号'
                } else {
                    await WarehouseDiskDeficient.create(data)
                }
            }
            if (error) {
                next('已存在该单据编号');
...
async function queryWarehouseDiskDeficient(req, res, next) {
    L('Calling API WarehouseDiskDeficient.queryWarehouseDiskDeficient', req.body);
    ...
    try {
        ...
        if (!data.filter || !data.filter.accountSetId) {
            ...
            if (data.isOne) {
                let result = await WarehouseDiskDeficient.findOne(data.filter).sort({createdAt: -1})
                res.status(200).json(result);
            } else {
                if (!data.pageInfo || !data.pageInfo.page || !data.pageInfo.limit) {
                    next(new errors.BadRequest('分页参数必填'));
                } else {
                    let result = await WarehouseDiskDeficient.find(data.filter).sort({createdAt: -1})
                      .limit(parseInt(data.pageInfo.limit))
                      .skip(data.pageInfo.limit * (data.pageInfo.page - 1));
                    let total = await WarehouseDiskDeficient.find(data.filter).countDocuments();
                    res.status(200).json({
                        total,
                        list: result
                    });
...
async function batchWarehouseDiskDeficient(req, res, next) {
    L('Calling API WarehouseDiskDeficient.batchWarehouseDiskDeficient', req.body);
    ...
    try {
        ...
        if (!data.ids || !data.ids.length) {
            ...
            if (data.action === '删除') {
                let result = await WarehouseDiskDeficient.find({_id: { $in: data.ids}})
                ...
                await WarehouseDiskDeficient.deleteMany({ _id: {$in: data.ids} }, (err) => {
                    if (err) {
                ...
                await WarehouseDiskDeficient.updateMany({_id: {$in: data.ids}}, { $set: data.action })
                res.status(200).end();
            }
        }
    } catch (err) {
...
async function parameterWarehouseDiskDeficient(req, res, next) {
    L('Calling API WarehouseDiskDeficient.parameterWarehouseDiskDeficient', req.body);
    ...
    try {
        ...
        if (!data.accountSetId) {
            ...
            let total = await WarehouseDiskDeficient.find(data).countDocuments();
            res.status(200).json({
                total
            });
        }
