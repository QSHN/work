...
const WarehouseIn = require('./warehouseIn.model');
...
module.exports = {
    ...
    queryWarehouseIn,
    batchWarehouseIn,
    parameterWarehouseIn
};

async function updateWarehouseIn(req, res, next) {
    L('Calling API WarehouseIn.updateWarehouseIn', req.body);
    ...
    try {
        ...
        if (!data.accountSetId) {
            ...
            delete data._id
            if (_id) {
                await WarehouseIn.updateOne({_id}, { $set: data })
            } else {
                let result = await WarehouseIn.findOne({
                    code: data.code
                })
                if (result) {
                    error = '已存在该单据编号'
                } else {
                    await WarehouseIn.create(data)
                }
            }
            if (error) {
                next('已存在该单据编号');
...
async function queryWarehouseIn(req, res, next) {
    L('Calling API WarehouseIn.queryWarehouseIn', req.body);
    ...
    try {
        ...
        if (!data.filter || !data.filter.accountSetId) {
            ...
            if (data.isOne) {
                let result = await WarehouseIn.findOne(data.filter).sort({createdAt: -1})
                res.status(200).json(result);
            } else {
                if (!data.pageInfo || !data.pageInfo.page || !data.pageInfo.limit) {
                    next(new errors.BadRequest('分页参数必填'));
                } else {
                    let result = await WarehouseIn.find(data.filter).sort({createdAt: -1})
                      .limit(parseInt(data.pageInfo.limit))
                      .skip(data.pageInfo.limit * (data.pageInfo.page - 1));
                    let total = await WarehouseIn.find(data.filter).countDocuments();
                    res.status(200).json({
                        total,
                        list: result
                    });
...
async function batchWarehouseIn(req, res, next) {
    L('Calling API WarehouseIn.batchWarehouseIn', req.body);
    ...
    try {
        ...
        if (!data.ids || !data.ids.length) {
            ...
            if (data.action === '删除') {
                let result = await WarehouseIn.find({_id: { $in: data.ids}})
                ...
                await WarehouseIn.deleteMany({ _id: {$in: data.ids} }, (err) => {
                    if (err) {
                ...
                await WarehouseIn.updateMany({_id: {$in: data.ids}}, { $set: data.action })
                res.status(200).end();
            }
        }
    } catch (err) {
...
async function parameterWarehouseIn(req, res, next) {
    L('Calling API WarehouseIn.parameterWarehouseIn', req.body);
    ...
    try {
        ...
        if (!data.accountSetId) {
            ...
            let total = await WarehouseIn.find(data).countDocuments();
            res.status(200).json({
                total
            });
        }
