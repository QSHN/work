...
import { queryWarehouseAllocatingIn, batchWarehouseAllocatingIn } from '../../api/warehouseAllocatingIn'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      queryWarehouseAllocatingIn(search).then(res => {
        let list = []
        res.data.list.forEach(f => {
          f.list.forEach(fl => {
            list.push({
              ...
              commodityWarehouseOut: fl.warehouseIdOut,
              commodityWarehouseIn: fl.warehouseId,
    ...
    batchFun (type, title, val = null) {
      ...
      selectList.forEach(id => {
        let fData = this.warehouseAllocatingIn.find(f => f._id === id)
        if (title === '删除') {
          if (fData.audit === '已审核') {
            errorList.push({
              code: fData.code,
