  ...
  <div>
    ...
    <Card style="margin-top: 10px;">
      ...
      <Table :height="getTableHeight"
        ...
        <template slot-scope="{ row }" slot="warehouseIdOut">
          <div class="text">{{ warehouseList.find(f => f._id === row.warehouseIdOut) && warehouseList.find(f => f._id === row.warehouseIdOut).name }}</div>
          <div class="hover_layer">
            <Select v-model="formData.list[row._index].warehouseIdOut">
...
} from '../../api/warehouseAllocatingIn'
...
export default {
  ...
  watch: {
      ...
      handler () {
        if (this.accountSet) {
          if (this.$route.query.id) {
            this.loadWarehouseAllocatingIn()
          } else {
            this.loadParameter()
          }
          this.loadDepartment()
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'warehouseAllocatingIn') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '调拨入库') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}调拨入库的权力，请联系管理员`)
            }
          }
        })
      }
    ...
    loadParameter () {
      ...
      parameterWarehouseAllocatingIn({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `DBRK-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          departmentIdOut: null,
          departmentIdIn: null,
          businessType: '同价调拨',
      ...
      this.$refs.formData.validate((valid) => {
        if (valid) {
          if (this.formData.list.length === 0) {
            ...
            this.$Message.error(`调拨入库单商品的标记红点的项必录，请检查！`)
          } else {
            updateWarehouseAllocatingIn(this.formData).then(res => {
    ...
    batchFun (title, id, val) {
      ...
      this.$Modal.confirm({
        title: `${title}调拨入库单`,
        content: `确定${title}调拨入库单`,
        onOk: () => {
          batchWarehouseAllocatingIn({
            action: { audit: val },
            ids: [id]
          }).then(res => {
            this.$Message.success(`成功${title}调拨入库单`)
            this.loadWarehouseAllocatingIn()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    updateList (list) {
      ...
      list.forEach(f => {
        f.list.forEach((fl, i) => {
          ...
          obj.pId = f._id
          obj.pIndex = i
          obj.number = NP.minus(obj.number || 0, obj.inNumber || 0)
          obj.warehouseId = fl.warehouseIdIn
          obj.warehouseIdOut = fl.warehouseId
          delete obj.warehouseIdIn
          result.push(obj)
        })
      })
    ...
    goNav (type) {
      let navList = this.warehouseAllocatingIn
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('warehouseAllocatingIn'))
      }
