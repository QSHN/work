    ...
    <Card style="margin-top: 10px;" v-if="isExpend">
      <Form label-position="top">
        <Row type="flex" style="margin-bottom: 10px;">
          ...
          <FormItem label="出库状态" style="margin: 0 30px 0 0;">
            <Select v-model="search.storage" style="width:200px">
              <Option v-for="(item, i) in ['全部', '未出库', '部分出库', '全部出库']" :value="item" :key="i">{{ item }}</Option>
            </Select>
          </FormItem>
...
import { trackingSalesReport } from '../../api/salesReport'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      this.loading = true
      trackingSalesReport(obj).then(res => {
        this.loading = false
        let list = []
        res.data.forEach(f => {
          f.list.forEach(fl => {
            list.push({
              ...
              commodityInStorage: fl.outStorage,
