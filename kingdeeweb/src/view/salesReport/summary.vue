...
import { summarySalesReport } from '../../api/salesReport'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      this.loading = true
      summarySalesReport(obj).then(res => {
        this.loading = false
        let list = []
        res.data.forEach(f => {
          let fData = null
          ...
          console.log(f.salesExpense)
          let salesExpense = NP.times(f.salesExpense.reduce((total, v) => (
            total = NP.plus(total, v.money || 0)
          ), 0), f.list.length)
          f.list.forEach(fl => {
            let price = NP.times((f.orderType === '销售退货单' ? fl.reStorage : fl.outStorage) || 0, fl.price || 0)
            let obj = {
              ...
              salesExpense: this.formatMoney(salesExpense, '', 2)
