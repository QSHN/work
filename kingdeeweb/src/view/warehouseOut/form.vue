        ...
        <Row type="flex">
          ...
          <FormItem label="业务类型" style="margin: 0 30px 0 0;">
            <Select v-model="formData.businessType" style="width:200px">
              <Option v-for="(item, index) in ['其他出库']" :value="item" :key="index">{{ item }}</Option>
            </Select>
          </FormItem>
        </Row>
      </Form>
...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'warehouseOut') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '其他出库') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}其他出库的权力，请联系管理员`)
            }
          }
        })
      }
    ...
    loadParameter () {
      ...
      parameterWarehouseOut({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `QTCK-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          customerId: null,
          clerkId: null,
          departmentId: null,
      ...
      this.$refs.formData.validate((valid) => {
        if (valid) {
          if (this.formData.list.length === 0) {
            ...
            this.$Message.error(`其他出库单商品的标记红点的项必录，请检查！`)
          } else if (this.formData.list.filter(f =>
            f.number > f.inventory
          ).length) {
            this.$Message.error(`请检查, 部分商品库存数量不足，无法出库`)
          } else {
            updateWarehouseOut(this.formData).then(res => {
    ...
    batchFun (title, id, val) {
      ...
      this.$Modal.confirm({
        title: `${title}其他出库单`,
        content: `确定${title}其他出库单`,
        onOk: () => {
          batchWarehouseOut({
            action: { audit: val },
            ids: [id]
          }).then(res => {
            this.$Message.success(`成功${title}其他出库单`)
            this.loadWarehouseOut()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    goNav (type) {
      let navList = this.warehouseOut
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('warehouseOut'))
      }
