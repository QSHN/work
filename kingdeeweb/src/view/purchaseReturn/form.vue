  ...
  <div>
    <Row type="flex" justify="space-between">
      <div style="display: flex; align-items: center;">
        <template v-if="$route.query.id">
          ...
          退款状态：
          <span :style="{marginRight: '20px', color: formData.refundStatus === '未退款' ? '#ed4014' : '#19be6b'}">
      ...
      <div>
        ...
        <Button style="margin-right: 10px;" v-if="formData.audit === '已审核'" :disabled="!$route.query.id" @click="payable">退款</Button>
    ...
    <Card style="margin-top: 10px;">
      ...
      <Table height="350"
        ...
        <template slot-scope="{ row }" slot="reStorage">
          <div class="text">{{ row.reStorage }}</div>
          <div class="hover_layer">
            <InputNumber
              v-model="formData.list[row._index].reStorage"
              :min="0"
              :step="1"
              style="width: 110px"
      ...
      <Row style="margin-top: 10px;">
        <span>支付退款</span>
        <Select v-model="formData.advanceAccountId" placeholder="退款账号" style="width: 150px; margin-left: 23px;">
        ...
        <Select v-model="formData.advanceType" placeholder="退款方式" style="width: 150px; margin-left: 10px;">
        ...
        <Button style="margin-left: 10px;" :disabled="true">多账户退款</Button>
        <span style="margin-left: 10px;">退款抹零</span>
      ...
      <Row type="flex" justify="end" align="middle" style="margin-top: 10px; border-top: 1px solid #dcdee2; padding-top: 10px;">
        ...
        <span>退款合计</span>
        <span style="margin: 0 20px 0 5px;">{{ formatMoney(formData.refund || 0, '', 2) }}</span>
        <span>本单未退</span>
        <span style="margin: 0 20px 0 5px;">{{ formatMoney(formData.unRefund || 0, '', 2) }}</span>
      </Row>
    </Card>
...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'purchaseReturn') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '采购退货') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}采购退货的权力，请联系管理员`)
            }
          }
        })
      }
    ...
    loadParameter () {
      ...
      parameterPurchaseReturn({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `CGTH-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          supplierId: null,
          clerkId: null,
          departmentId: null,
    ...
    loadPurchaseReturn () {
      queryPurchaseReturn({
        ...
        this.isCanEdit = res.data.audit === '未审核' && res.data.refundStatus === '未退款'
      }).catch(err => {
        this.$Notice.error({
          title: '采购入库',
          desc: err.message
      ...
      this.$refs.formData.validate((valid) => {
        if (valid) {
          if (this.formData.list.length === 0) {
            ...
            this.$Message.error(`采购退货商品的标记红点的项必录，请检查！`)
          } else {
            updatePurchaseReturn(this.formData).then(res => {
    ...
    changePrice (index) {
      ...
      if (data.price !== null) {
        ...
        if (data.reStorage !== null) {
          this.$set(this.formData.list[index], 'payable', NP.times(data.reStorage, NP.times(data.price, NP.divide(data.taxRate, 100))))
          this.$set(this.formData.list[index], 'payableTotal', NP.times(data.unitPrice, data.reStorage))
        }
      }
      if (data.packageNumber && data.reStorage !== data.packageNumber) {
    ...
    changePackage (index) {
      ...
      if (data.packageList && data.packageList.length) {
        ...
        if (number > 0) {
          ...
          this.$set(this.formData.list[index], 'reStorage', number)
    ...
    handleSummary ({ columns, data }) {
      ...
      columns.forEach((column, index) => {
        ...
        if (!values.every(value => isNaN(value))) {
          ...
          sums[key] = {
            key,
            value: key === 'reStorage' ? v : this.formatMoney(v, '', 2)
          }
          if (key === 'payableTotal') {
            this.$set(this.formData, 'orderMoney', NP.minus(v || 0, this.formData.discount || 0))
            this.$set(this.formData, 'unRefund', NP.minus(this.formData.orderMoney || 0, this.formData.refund || 0))
          }
        } else {
          sums[key] = {
            key,
    ...
    advanceMoney () {
      this.$set(this.formData, 'unRefund', NP.plus(this.formData.advanceMoney || 0, this.formData.maLing || 0))
    },
    getPriceAuto (row) {
    ...
    batchFun (title, id, val) {
      ...
      if (title === '反审核') {
        if (this.formData.audit === '未审核') {
          ...
          return
        } else if (this.formData.refundStatus !== '未退款') {
          this.$Message.error('该单据退款状态不允许反审核')
          return
      ...
      this.$Modal.confirm({
        title: `${title}采购退货单`,
        content: `确定${title}采购退货单`,
        onOk: () => {
          batchPurchaseReturn({
            action: { audit: val },
            ids: [id]
          }).then(res => {
            this.$Message.success(`成功${title}采购退货单`)
            this.loadPurchaseReturn()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    updateList (list) {
      ...
      list.forEach(f => {
        ...
        f.list.forEach((fl, i) => {
          ...
          obj.pId = f._id
          obj.pIndex = i
          obj.reStorage = NP.minus(obj.number || 0, obj.reStorage || 0)
          result.push(obj)
        })
      })
    ...
    payable () {
      ...
      let error = ''
      if (this.formData.audit === '未审核') {
        error += `<P>该单据未审核，无法退款</P>`
      }
      if (this.formData.refundStatus === '全部退款') {
        error += `<P>该单据应收金额已全部退款</P>`
      }
      if (error) {
        this.$Modal.error({
          title: '退款',
    ...
    goNav (type) {
      let navList = this.purchaseReturn
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('purchaseReturn'))
      }
