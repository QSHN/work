<template>
  <div>
    ...
    <Row type="flex" style="margin-top: 10px;">
      ...
      <Button style="margin-right: 10px;" :disabled="!selectList.length" @click="payable">退款</Button>
...
import { queryPurchaseReturn, batchPurchaseReturn } from '../../api/purchaseReturn'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      queryPurchaseReturn(search).then(res => {
        let list = []
    ...
    batchFun (type, title, val = null) {
      ...
      selectList.forEach(id => {
        let fData = this.purchaseReturn.find(f => f._id === id)
        if (title === '删除') {
          if (fData.audit === '已审核') {
            errorList.push({
              code: fData.code,
