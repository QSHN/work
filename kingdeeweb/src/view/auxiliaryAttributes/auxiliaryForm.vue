<template>
  <Modal
    ...
    <Form ref="formData" :model="formData" :rules="formDataRule" :label-width="120" style="margin-top: 10px;">
      ...
      <FormItem label="辅助属性组" prop="groupId">
...
<script>
import { updateAuxiliaryAttributes } from '../../api/auxiliaryAttributes'
export default {
  name: 'currency',
  props: {
    groupList: Array
  ...
  data () {
    return {
      ...
      formDataRule: {
        ...
        groupId: [
          { required: true, message: '辅助属性组不能为空', trigger: 'change' }
        ]
      },
      auxiliaryList: []
    }
  ...
  methods: {
    ...
    sure () {
      this.$refs.formData.validate((valid) => {
        if (valid) {
          updateAuxiliaryAttributes(this.formData).then(res => {
            this.$Notice.success({
              title: '辅助属性',
              desc: `成功${this.type}辅助属性`
            })
            this.$emit('load')
            this.cancel()
          }).catch(err => {
