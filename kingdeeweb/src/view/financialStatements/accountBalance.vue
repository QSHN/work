  ...
  <div>
    ...
    <Card shadow style="margin: 10px 0;">
      <Row type="flex">
        ...
        <div style="display: flex; margin-left: 20px;">
          <span style="margin-right: 5px; line-height: 30px;">显示核算明细账：</span>
          <Checkbox v-model="search.detail" style="margin-top: 7px;"></Checkbox>
        </div>
      ...
      <div class="c_table">
        ...
        <div class="t_c">
          <ul v-for="(v, i) in tableData" :key="i">
            <li :class="{active: which[0] === i && which[1] === -1}"
                @click="which = [i, -1]"
                @dblclick="goDetail(v, null)">
              <div class="tb">{{v.code}}</div>
              <div class="tb tb2" :style="{paddingLeft: v.parentId.length * 10 + 'px'}">{{v.name}}</div>
              <div class="tb">{{v.mnemonic}}</div>
              <div class="tb tb3">
                <div class="tb_c">
              ...
              <div class="tb tb3" style="text-align: right;">
                <div class="tb_c">
                  <div class="tb" style="text-align: right">{{v.yearBorrow !== 0 ? formatMoney(v.yearBorrow, '', 2) : ''}}</div>
                  <div class="tb" style="text-align: right">{{v.yearCredit !== 0 ? formatMoney(v.yearCredit, '', 2) : ''}}</div>
                </div>
              </div>
              <div class="tb tb3" style="text-align: right;">
                <div class="tb_c">
            ...
            <li v-for="(v1, i1) in v.child" :key="i1"
                @click="which = [i, i1]"
                @dblclick="goDetail(v, v1)"
                :class="{active: which[0] === i && which[1] === i1, no_border: true}">
              <div class="tb">{{v1.code}}</div>
              <div class="tb tb2" :style="{paddingLeft: v.parentId.length * 10 + 20 + 'px'}">{{v1.name}}</div>
              <div class="tb"></div>
              <div class="tb tb3">
                <div class="tb_c">
                  <template v-if="v.direction === '借方'">
                    <div class="tb" style="text-align: right">{{v1.beginningBalance > 0 ? formatMoney(Math.abs(v1.beginningBalance), '', 2) : ''}}</div>
                    <div class="tb" style="text-align: right">{{0 > v1.beginningBalance ? formatMoney(Math.abs(v1.beginningBalance), '', 2) : ''}}</div>
                  </template>
                  <template v-else>
                    <div class="tb" style="text-align: right">{{0 > v1.beginningBalance ? formatMoney(Math.abs(v1.beginningBalance), '', 2) : ''}}</div>
                    <div class="tb" style="text-align: right">{{v1.beginningBalance > 0 ? formatMoney(Math.abs(v1.beginningBalance), '', 2) : ''}}</div>
              ...
              <div class="tb tb3" style="text-align: right;">
                <div class="tb_c">
                  <div class="tb" style="text-align: right">{{v1.borrower !== 0 ? formatMoney(v1.borrower, '', 2) : ''}}</div>
                  <div class="tb" style="text-align: right">{{v1.lender !== 0 ? formatMoney(v1.lender, '', 2) : ''}}</div>
                </div>
              </div>
              <div class="tb tb3" style="text-align: right;">
                <div class="tb_c">
                  <div class="tb" style="text-align: right">{{v1.yearBorrow !== 0 ? formatMoney(v1.yearBorrow, '', 2) : ''}}</div>
                  <div class="tb" style="text-align: right">{{v1.yearCredit !== 0 ? formatMoney(v1.yearCredit, '', 2) : ''}}</div>
                </div>
              </div>
              <div class="tb tb3" style="text-align: right;">
                <div class="tb_c">
                  <template v-if="v.direction === '借方'">
                    <div class="tb" style="text-align: right">{{v1.balance > 0 ? formatMoney(Math.abs(v1.balance), '', 2) : ''}}</div>
                    <div class="tb" style="text-align: right">{{0 > v1.balance ? formatMoney(Math.abs(v1.balance), '', 2) : ''}}</div>
                  </template>
                  <template v-else>
                    <div class="tb" style="text-align: right">{{0 > v1.balance ? formatMoney(Math.abs(v1.balance), '', 2) : ''}}</div>
                    <div class="tb" style="text-align: right">{{v1.balance > 0 ? formatMoney(Math.abs(v1.balance), '', 2) : ''}}</div>
    ...
    <printModal ref="printModal" :printList="tableData" :printType="'accountBalance'" :period="search" />
  </div>
</template>
    ...
    goDetail (v, v1) {
      this.$router.push({
        ...
        query: {
          ...
          subjectCode: v.code,
          subject: v.name,
          accountingPeriod: this.moment(this.search.accountingPeriod).format('YYYY-MM-DD'),
          accountingPeriodEnd: this.moment(this.search.accountingPeriodEnd).format('YYYY-MM-DD'),
          accountingItem: v1 ? v1.name : 'null'
        }
      })
    },
    loadCurrency () {
