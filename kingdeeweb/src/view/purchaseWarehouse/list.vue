<template>
  <div>
    ...
    <Row type="flex" style="margin-top: 10px;">
      ...
      <Button style="margin-right: 10px;" :disabled="!selectList.length" @click="payable">付款</Button>
...
import { queryPurchaseWarehouse, batchPurchaseWarehouse } from '../../api/purchaseWarehouse'
  ...
  methods: {
    ...
    batchFun (type, title, val = null) {
      ...
      selectList.forEach(id => {
        let fData = this.purchaseWarehouse.find(f => f._id === id)
        if (title === '删除') {
          if (fData.audit === '已审核') {
            errorList.push({
              code: fData.code,
          ...
          } else if (fData.paymentStatus !== '未付款') {
            errorList.push({
              code: fData.code,
    ...
    payable () {
      ...
      selectList.forEach(id => {
        ...
        if (fData.paymentStatus === '全部付款') {
          error += `<P>${fData.code}应收金额已全部付款</P>`
        }
        if (supplierId) {
