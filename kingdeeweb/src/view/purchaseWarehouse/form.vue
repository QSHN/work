  ...
  <div>
    <Row type="flex" justify="space-between">
      <div style="display: flex; align-items: center;">
        <template v-if="$route.query.id">
          ...
          付款状态：
          <span :style="{marginRight: '20px', color: formData.paymentStatus === '未付款' ? '#ed4014' : '#19be6b'}">
      ...
      <div>
        ...
        <Button style="margin-right: 10px;" :disabled="formData.audit !== '已审核'" @click="payable">付款</Button>
    ...
    <Card style="margin-top: 10px;">
      <Row>
        ...
        <span style="font-size: 12px; margin-right: 5px;">预计采购费用: {{ formatMoney(getPurchaseExpense, '', 2) }}</span>
        <Button size="small" @click="purchaseExpense(false)">更改</Button>
      </Row>
      ...
      <Table height="350"
          ...
          <div class="hover_layer">
            <Select v-model="formData.list[row._index].warehouseId">
              ...
              <Option v-for="item in warehouseList" :value="item._id" :key="item._id">
                {{ item.name }}
                ( 库存数量: {{ getInventory(warehouseList.find(f => f._id === item._id), row) }} )
              </Option>
            </Select>
          </div>
        </template>
        ...
        <template slot-scope="{ row }" slot="inventory">
          {{ getInventory(warehouseList.find(f => f._id === row.warehouseId), row, false) }}
        </template>
        ...
        <template slot-scope="{ row }" slot="inStorage">
          <div class="text">{{ row.inStorage }}</div>
          <div class="hover_layer">
            <InputNumber
              v-model="formData.list[row._index].inStorage"
              :min="0"
              :step="1"
              style="width: 110px"
      ...
      <Row style="margin-top: 10px;">
        ...
        <Button style="margin-left: 10px;" @click="selectPayable(null)">选源单</Button>
      </Row>

      <Row style="margin-top: 10px;">
        <span>支付货款</span>
        <Select v-model="formData.advanceAccountId" placeholder="付款账号" style="width: 150px; margin-left: 23px;">
        ...
        <Select v-model="formData.advanceType" placeholder="付款方式" style="width: 150px; margin-left: 10px;">
        ...
        <Button style="margin-left: 10px;" :disabled="true">多账户付款</Button>
        <span style="margin-left: 10px;">付款抹零</span>
    ...
    <advanceModal ref="advanceModal" :supplierList="supplierList" :spendingList="spendingList" @purchaseExpense="purchaseExpense" @loadSpendingCategory="loadSpendingCategory" @loadSupplier="loadSupplier"  />
    <orderModel ref="orderModel" @selectOrder="selectOrder" />
    <commodityModal ref="commodityModal" @addCommodity="addCommodity" />
    <payableModel ref="payableModel" @selectPayable="selectPayable" :hasList="formData.offsetList" :formData="formData" />
...
} from '../../api/purchaseWarehouse'
import { queryPurchaseOrder } from '../../api/purchaseOrder'
...
export default {
  ...
  computed: {
    ...
    getPurchaseExpense () {
      let money = 0
      if (this.formData.purchaseExpense.length) {
        money = this.formData.purchaseExpense.reduce((total, v) => (
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'purchaseWarehouse') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '采购入库') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}采购入库的权力，请联系管理员`)
            }
          }
        })
      }
    ...
    loadParameter () {
      ...
      parameterPurchaseWarehouse({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `CGRK-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          supplierId: null,
          clerkId: null,
          departmentId: null,
    ...
    loadPurchaseWarehouse () {
      queryPurchaseWarehouse({
        ...
        this.isCanEdit = res.data.audit === '未审核' && res.data.paymentStatus === '未付款'
      }).catch(err => {
        this.$Notice.error({
          title: '采购入库',
          desc: err.message
      ...
      this.$refs.formData.validate((valid) => {
        if (valid) {
          if (this.formData.list.length === 0) {
            ...
            this.$Message.error(`采购商品的标记红点的项必录，请检查！`)
          } else {
            updatePurchaseWarehouse(this.formData).then(res => {
    ...
    purchaseExpense (changeList) {
      if (changeList) {
        this.formData.purchaseExpense = JSON.parse(JSON.stringify(changeList))
      } else {
        if (this.formData.supplierId) {
          this.$refs.advanceModal.show(this.formData.purchaseExpense, this.formData.supplierId)
        } else {
    ...
    changePrice (index) {
      ...
      if (data.price !== null) {
        ...
        if (data.inStorage !== null) {
          this.$set(this.formData.list[index], 'payable', NP.times(data.inStorage, NP.times(data.price, NP.divide(data.taxRate, 100))))
          this.$set(this.formData.list[index], 'payableTotal', NP.times(data.unitPrice, data.inStorage))
          this.$set(this.formData.list[index], 'unStorage', NP.minus(NP.minus(data.number || 0, data.inStorage || 0), data.reStorage || 0))
          this.$set(this.formData.list[index], 'unitDelivery', data.price)
          this.$set(this.formData.list[index], 'deliveryCost', NP.times(data.price, data.inStorage))
        }
      }
      if (data.packageNumber && data.inStorage !== data.packageNumber) {
    ...
    changePackage (index) {
      ...
      if (data.packageList && data.packageList.length) {
        ...
        if (number > 0) {
          ...
          this.$set(this.formData.list[index], 'inStorage', number)
    ...
    handleSummary ({ columns, data }) {
      ...
      columns.forEach((column, index) => {
        ...
        if (!values.every(value => isNaN(value))) {
          ...
          sums[key] = {
            key,
            value: key === 'inStorage' ? v : this.formatMoney(v, '', 2)
          }
          if (key === 'payableTotal') {
    ...
    batchFun (title, id, val) {
      ...
      if (title === '反审核') {
        if (this.formData.audit === '未审核') {
          ...
          return
        } else if (this.formData.paymentStatus !== '未付款') {
          this.$Message.error('该单据付款状态不允许反审核')
          return
          ...
          this.$Message.error('该单据存在采购退货单')
          return
        }
      }
      this.$Modal.confirm({
        title: `${title}采购入库单`,
        content: `确定${title}采购入库单`,
        onOk: () => {
          batchPurchaseWarehouse({
            action: { audit: val },
            ids: [id]
          }).then(res => {
            this.$Message.success(`成功${title}采购入库单`)
            this.loadPurchaseWarehouse()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    selectPayable (changeList) {
      if (changeList) {
        ...
        if (this.formData.supplierId) {
          this.$refs.payableModel.show(this.formData.supplierId)
        } else {
          this.$Message.error('请选择供应商')
        }
      }
    ...
    updateList (list) {
      let result = []
      list.forEach(f => {
        this.formData.supplierId = f.supplierId
        ...
        f.list.forEach((fl, i) => {
          ...
          obj.pId = f._id
          obj.pIndex = i
          obj.inStorage = NP.minus(obj.number || 0, obj.inStorage || 0)
    ...
    payable () {
      ...
      let error = ''
      if (this.formData.audit === '未审核') {
        error += `<P>该单据未审核，无法付款</P>`
      }
      if (this.formData.paymentStatus === '全部付款') {
        error += `<P>该单据应收金额已全部付款</P>`
      }
      if (error) {
        this.$Modal.error({
          title: '付款',
    ...
    goNav (type) {
      let navList = this.purchaseWarehouse()
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('purchaseWarehouse'))
      }
