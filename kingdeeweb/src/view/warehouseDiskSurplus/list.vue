...
import { queryWarehouseDiskSurplus, batchWarehouseDiskSurplus } from '../../api/warehouseDiskSurplus'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      queryWarehouseDiskSurplus(search).then(res => {
    ...
    batchFun (type, title, val = null) {
      ...
      selectList.forEach(id => {
        let fData = this.warehouseDiskSurplus.find(f => f._id === id)
        if (title === '删除') {
          if (fData.audit === '已审核') {
            errorList.push({
              code: fData.code,
