...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'warehouseDiskSurplus') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '盘盈单') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}盘盈单的权力，请联系管理员`)
            }
          }
        })
      }
      ...
      this.$refs.formData.validate((valid) => {
        if (valid) {
          if (this.formData.list.length === 0) {
            ...
            this.$Message.error(`盘盈商品的标记红点的项必录，请检查！`)
          } else {
            ...
            updateWarehouseDiskSurplus(this.formData).then(res => {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          batchWarehouseDiskSurplus({
            ...
            this.$Message.success(`成功${title}盘盈单`)
            this.loadWarehouseDiskSurplus()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    goNav (type) {
      let navList = this.warehouseDiskSurplus
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('warehouseDiskSurplus'))
      }
