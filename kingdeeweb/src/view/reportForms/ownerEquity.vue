  ...
  <div>
    ...
    <Table
      ...
      <template slot-scope="{ row }" slot="index">
        <template v-if="row._index === 0 || row._index === 1 || isShowData">
          {{ row._index + 1 }}
        </template>
        <template v-else>
          ...
          <div class="hover_layer" style="padding-left: 0">
            <div class="btn_group">
              <div class="add" @click="editRow(false, 'ownerEquity', tableData, row._index)"><Icon type="md-add-circle" /></div>
              <div class="delete" @click="editRow(true, 'ownerEquity', tableData, row._index)"><Icon type="md-close-circle" /></div>
            </div>
          </div>
        </template>
      </template>
      ...
      <template slot-scope="{ row, column }" slot="content">
        <div v-if="(row._index === 0 || row._index === 1 || tableData[0][column.title].type === 'title')"
             :class="{change: !isShowData}"
             :style="{ textAlign: row[column.title].align, padding: `2px 5px 2px ${row[column.title].padding}px` }"
             @click="editTitle('ownerEquity', tableData, row, column.title)">
        ...
        <template v-else-if="tableData[0][column.title].type === 'formula'">
          <div v-if="!isShowData"
               ...
               @click="editFormula('ownerEquity', tableData, row, column.title)">
          </div>
    ...
    <printModal ref="printModal" :printList="getPrint" :printType="'ownerEquity'" :period="search" />
  </div>
</template>
            ...
            renderHeader: (h, params) => {
              return h('div', {
                ...
                h('Icon', {
                  ...
                  on: {
                    click: (val) => {
                      this.editColumn(false, 'ownerEquity', this.tableData, params.column.title)
                    }
                  }
                }),
                h('Icon', {
                  ...
                  on: {
                    click: (val) => {
                      this.editColumn(true, 'ownerEquity', this.tableData, params.column.title)
                    }
                  }
                }),
