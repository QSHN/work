<template>
  <div>
    ...
    <Table
      ...
      <template slot-scope="{ row }" slot="index">
        ...
        <template v-else>
          ...
          <div class="hover_layer" style="padding-left: 0">
            <div class="btn_group">
              <div class="add" @click="editRow(false, 'profit', tableData, row._index)"><Icon type="md-add-circle" /></div>
              <div class="delete" @click="editRow(true, 'profit', tableData, row._index)"><Icon type="md-close-circle" /></div>
            </div>
          </div>
        </template>
      </template>
      ...
      <template slot-scope="{ row, column }" slot="content">
        <div v-if="(row._index === 0 || tableData[0][column.title].type === 'title')"
             ...
             @click="editTitle('profit', tableData, row, column.title)">
        ...
        <template v-else-if="tableData[0][column.title].type === 'formula'">
          <div v-if="!isShowData"
               ...
               @click="editFormula('profit', tableData, row, column.title)">
          </div>
    ...
    <printModal ref="printModal" :printList="getPrint" :printType="'profit'" :period="search" />
  </div>
</template>
            ...
            renderHeader: (h, params) => {
              return h('div', {
                ...
                h('Icon', {
                  ...
                  on: {
                    click: (val) => {
                      this.editColumn(false, 'profit', this.tableData, params.column.title)
                    }
                  }
                }),
                h('Icon', {
                  ...
                  on: {
                    click: (val) => {
                      this.editColumn(true, 'profit', this.tableData, params.column.title)
                    }
                  }
                }),
