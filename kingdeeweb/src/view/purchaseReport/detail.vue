  ...
  <div>
    ...
    <Table
      ...
      <template slot-scope="{ row }" slot="code">
        <router-link :to="{
          name: row.orderType === '采购入库单' ? 'purchaseWarehouse' : 'purchaseReturn',
          query: { id: row._id }
    ...
    <printModal
      ref="printModal"
      :printList="tableData"
      :printType="'purchaseReportDetail'"
...
import { detailPurchaseReport } from '../../api/purchaseReport'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      this.loading = true
      detailPurchaseReport(obj).then(res => {
        this.loading = false
        let list = []
        res.data.forEach(f => {
          f.list.forEach(fl => {
            list.push({
              ...
              commodityNumber: f.orderType === '采购退货单' ? fl.reStorage : fl.inStorage,
