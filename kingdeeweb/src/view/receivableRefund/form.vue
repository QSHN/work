...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'receivableRefund') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '预收退款') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}预收退款的权力，请联系管理员`)
            }
          }
        })
      }
    ...
    loadParameter () {
      ...
      parameterReceivableRefund({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `YSKTKD-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          customerId: null,
          clerkId: null,
          departmentId: null,
          ...
          if (error) {
            ...
            if (error) {
              ...
              if (this.formData.order.total !== this.formData.main.total) {
                ...
                updateReceivableRefund(this.formData).then(res => {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          batchReceivableRefund({
            ...
            this.$Message.success(`成功${title}预收退款单`)
            this.loadReceivableRefund()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    goNav (type) {
      let navList = this.receivableRefund
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('receivableRefund'))
      }
