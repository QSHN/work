...
import { queryReceivableCollection, batchReceivableCollection } from '../../api/receivableCollection'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      queryReceivableCollection(search).then(res => {
        let list = []
    ...
    batchFun (type, title, val = null) {
      ...
      selectList.forEach(id => {
        let fData = this.receivableCollection.find(f => f._id === id)
        if (title === '删除') {
          if (fData.audit === '已审核') {
            errorList.push({
              code: fData.code,
