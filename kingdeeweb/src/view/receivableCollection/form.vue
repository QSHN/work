...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'receivableCollection') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '收款') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}收款的权力，请联系管理员`)
            }
          }
        })
      }
    ...
    loadParameter () {
      ...
      parameterReceivableCollection({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `SKD-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          customerId: null,
          clerkId: null,
          departmentId: null,
        ...
        if (this.$route.query.pId && this.$route.query.type && this.$route.query.customerId) {
          ...
          if (this.formData.main.list.length === 0) {
            ...
            this.formData.main.list.forEach((v, i) => {
              if (!v.advanceAccountId || !v.advanceMoney || !v.advanceType) {
                error = `收款信息第${i + 1}条收款账户、收款方式和收款金额必填`
              }
            })
          }
          if (error) {
            ...
            if (error) {
              ...
              if (this.formData.order.total !== this.formData.main.total) {
                this.$Message.error('核销总金额不等于本次收款总金额')
              } else {
                ...
                updateReceivableCollection(this.formData).then(res => {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          batchReceivableCollection({
            ...
            this.$Message.success(`成功${title}收款单`)
            this.loadReceivableCollection()
          }).catch(err => {
            this.$Message.error(err.message)
          })
      ...
      columns.forEach((column, index) => {
        ...
        if (!values.every(value => isNaN(value))) {
          ...
          if (key === 'thisPay') {
            ...
            this.$set(this.formData.main, 'total', this.formatMoney(NP.plus(v || 0, this.formData.main.preferential || 0), '', 2))
          }
        } else {
          sums[key] = {
            key,
    ...
    goNav (type) {
      let navList = this.receivableCollection
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('receivableCollection'))
      }
