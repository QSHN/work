...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'payablePayments') return
        if (this.accountSet) {
  ...
  methods: {
    ...
    loadParameter () {
      ...
      parameterPayablePayments({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `YFKD-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          supplierId: null,
          clerkId: null,
          departmentId: null,
        ...
        if (valid) {
          ...
          if (error) {
            ...
            if ((this.formData.main.payTotal || 0) > this.formData.main.total) {
              error = `付款金额总和小于核销总额，付款金额总和：${this.formData.main.total}，核销总额：${this.formData.main.payTotal}`
              this.$Message.error(error)
            } else {
              ...
              updatePayablePayments(this.formData).then(res => {
    ...
    goNav (type) {
      let navList = this.payablePayments
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('payablePayments'))
      }
