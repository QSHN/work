...
import { queryWarehouseReceipt, batchWarehouseReceipt } from '../../api/warehouseReceipt'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      queryWarehouseReceipt(search).then(res => {
        let list = []
    ...
    batchFun (type, title, val = null) {
      ...
      selectList.forEach(id => {
        let fData = this.warehouseReceipt.find(f => f._id === id)
        if (title === '删除') {
          if (fData.audit === '已审核') {
            errorList.push({
              code: fData.code,
