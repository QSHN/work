...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'warehouseReceipt') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '移仓单') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}移仓单的权力，请联系管理员`)
            }
          }
        })
      }
    ...
    loadParameter () {
      ...
      parameterWarehouseReceipt({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `YCD-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          departmentIdOut: null,
          departmentIdIn: null,
          list: [],
      ...
      this.$refs.formData.validate((valid) => {
        if (valid) {
          if (this.formData.list.length === 0) {
            ...
            this.$Message.error(`移仓单的标记红点的项必录，请检查！`)
          } else {
            updateWarehouseReceipt(this.formData).then(res => {
    ...
    getInventory (warehouse, row, isUpdate = true) {
      ...
      if (isUpdate && warehouse && row.warehouseIdIn === warehouse._id) this.formData.list[row._index].inventory = num
      return num
    },
    changePrice (index) {
    ...
    batchFun (title, id, val) {
      ...
      this.$Modal.confirm({
        title: `${title}移仓单`,
        content: `确定${title}移仓单`,
        onOk: () => {
          batchWarehouseReceipt({
            action: { audit: val },
            ids: [id]
          }).then(res => {
            this.$Message.success(`成功${title}移仓单`)
            this.loadWarehouseReceipt()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    goNav (type) {
      let navList = this.warehouseReceipt
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('warehouseReceipt'))
      }
