...
import { queryWarehouseDiskDeficient, batchWarehouseDiskDeficient } from '../../api/warehouseDiskDeficient'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      queryWarehouseDiskDeficient(search).then(res => {
    ...
    batchFun (type, title, val = null) {
      ...
      selectList.forEach(id => {
        let fData = this.warehouseDiskDeficient.find(f => f._id === id)
        if (title === '删除') {
          if (fData.audit === '已审核') {
            errorList.push({
              code: fData.code,
        ...
        onOk: () => {
          if (canList.length) {
            ...
            if (title === '删除') {
              ...
              action[type] = val
              action['auditor'] = this.myUser._id
            }
