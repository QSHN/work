...
} from '../../api/warehouseDiskDeficient'
...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'warehouseDiskDeficient') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '盘亏单') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}盘亏单的权力，请联系管理员`)
            }
          }
        })
      }
      ...
      this.$refs.formData.validate((valid) => {
        if (valid) {
          if (this.formData.list.length === 0) {
            ...
            this.$Message.error(`盘亏商品的标记红点的项必录，请检查！`)
          } else {
            ...
            updateWarehouseDiskDeficient(this.formData).then(res => {
    ...
    batchFun (title, id, val) {
      ...
      this.$Modal.confirm({
        title: `${title}盘亏单`,
        content: `确定${title}盘亏单`,
        onOk: () => {
          batchWarehouseDiskDeficient({
            action: {
              audit: val,
              auditor: this.myUser._id
            },
            ids: [id]
          }).then(res => {
            this.$Message.success(`成功${title}盘亏单`)
            this.loadWarehouseDiskDeficient()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    goNav (type) {
      let navList = this.warehouseDiskDeficient
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('warehouseDiskDeficient'))
      }
