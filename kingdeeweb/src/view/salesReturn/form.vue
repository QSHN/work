  ...
  <div>
    <Row type="flex" justify="space-between">
      ...
      <div>
        ...
        <Button style="margin-right: 10px;" v-if="formData.audit === '已审核'" :disabled="!$route.query.id" @click="receivable">退款</Button>
...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'salesReturn') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '销售退货') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}销售退货的权力，请联系管理员`)
            }
          }
        })
      }
    ...
    loadParameter () {
      ...
      parameterSalesReturn({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `XSTH-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          customerId: null,
          clerkId: null,
          departmentId: null,
      ...
      this.$refs.formData.validate((valid) => {
        if (valid) {
          if (this.formData.list.length === 0) {
            ...
            this.$Message.error(`销售退货商品的标记红点的项必录，请检查！`)
          } else {
            updateSalesReturn(this.formData).then(res => {
    ...
    batchFun (title, id, val) {
      ...
      this.$Modal.confirm({
        title: `${title}销售退货单`,
        content: `确定${title}销售退货单`,
        onOk: () => {
          batchSalesReturn({
            action: { audit: val },
            ids: [id]
          }).then(res => {
            this.$Message.success(`成功${title}销售退货单`)
            this.loadSalesReturn()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    goNav (type) {
      let navList = this.salesReturn
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('salesReturn'))
      }
