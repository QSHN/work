<template>
  <div>
    ...
    <Row type="flex" style="margin-top: 10px;">
      ...
      <Button style="margin-right: 10px;" :disabled="!selectList.length" @click="receivable">退款</Button>
    ...
    <Table :width="getTableWidth"
      ...
      <template slot-scope="{ row }" slot="refundStatus">
        <Tag color="blue" v-if="row.refundStatus === '部分退款'">部分退款</Tag>
        <Tag color="green" v-else-if="row.refundStatus === '全部退款'">全部退款</Tag>
        <Tag color="red" v-else>未退款</Tag>
      </template>
...
import { querySalesReturn, batchSalesReturn } from '../../api/salesReturn'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      querySalesReturn(search).then(res => {
        let list = []
    ...
    batchFun (type, title, val = null) {
      ...
      selectList.forEach(id => {
        let fData = this.salesReturn.find(f => f._id === id)
        if (title === '删除') {
          if (fData.audit === '已审核') {
            errorList.push({
              code: fData.code,
          ...
          } else if (fData.refundStatus !== '未退款') {
            errorList.push({
              code: fData.code,
              val: fData.refundStatus
            })
          } else {
            canList.push({
              code: fData.code,
    ...
    receivable () {
      ...
      selectList.forEach(id => {
        ...
        if (fData.refundStatus === '全部退款') {
          error += `<P>${fData.code}应收金额已全部退款</P>`
        }
        if (customerId) {
          if (customerId !== fData.customerId) error += `<P>${fData.code}退款只能是同一个客户</P>`
        } else {
