...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'payableCollection') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '付款') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}付款的权力，请联系管理员`)
            }
          }
        })
      }
    ...
    loadParameter () {
      ...
      parameterPayableCollection({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `FKD-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          supplierId: null,
          clerkId: null,
          departmentId: null,
        ...
        if (this.$route.query.pId && this.$route.query.type && this.$route.query.supplierId) {
          this.formData.supplierId = this.$route.query.supplierId
          ...
          if (this.formData.main.list.length === 0) {
            ...
            this.formData.main.list.forEach((v, i) => {
              if (!v.advanceAccountId || !v.advanceMoney || !v.advanceType) {
                error = `付款信息第${i + 1}条付款账户、付款方式和付款金额必填`
              }
            })
          }
          if (error) {
            ...
            if (error) {
              ...
              if (this.formData.order.total !== this.formData.main.total) {
                this.$Message.error('核销总金额不等于本次付款总金额')
              } else {
                ...
                updatePayableCollection(this.formData).then(res => {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          batchPayableCollection({
            ...
            this.$Message.success(`成功${title}付款单`)
            this.loadPayableCollection()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    goNav (type) {
      let navList = this.payableCollection
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('payableCollection'))
      }
