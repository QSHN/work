...
import { queryPayableCollection, batchPayableCollection } from '../../api/payableCollection'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      queryPayableCollection(search).then(res => {
        let list = []
    ...
    batchFun (type, title, val = null) {
      ...
      selectList.forEach(id => {
        let fData = this.payableCollection.find(f => f._id === id)
        if (title === '删除') {
          if (fData.audit === '已审核') {
            errorList.push({
              code: fData.code,
