<template>
  <div>
    ...
    <Row type="flex" style="margin-top: 10px;">
      ...
      <Button style="margin-right: 10px;" :disabled="!selectList.length || selectList.length > 1" @click="pwFun">入库</Button>
      <Button style="margin-right: 10px;" :disabled="selectList.length <= 1" @click="pwFun">合并入库</Button>
    ...
    <Table :width="getTableWidth"
      ...
      <template slot-scope="{ row }" slot="storage">
        <Tag color="blue" v-if="row.storage === '部分入库'">部分入库</Tag>
        <Tag color="green" v-else-if="row.storage === '全部入库'">全部入库</Tag>
        <Tag color="red" v-else>未入库</Tag>
      </template>
...
import { queryPurchaseOrder, batchPurchaseOrder } from '../../api/purchaseOrder'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      queryPurchaseOrder(search).then(res => {
        let list = []
        ...
        this.purchaseOrder = res.data.list
        this.loading = false
    ...
    batchFun (type, title, val = null) {
      ...
      selectList.forEach(id => {
        let fData = this.purchaseOrder.find(f => f._id === id)
        if (title === '删除') {
          if (fData.audit === '已审核') {
            errorList.push({
              code: fData.code,
