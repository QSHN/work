    ...
    <Row type="flex" justify="space-between">
      <div style="display: flex; align-items: center;">
        <template v-if="$route.query.id">
          ...
          入库状态：
          <span :style="{marginRight: '20px', color: formData.storage === '未入库' ? '#ed4014' : '#19be6b'}">
            {{ formData.storage }}
          </span>
        </template>
      </div>
      <div>
        ...
        <Button style="margin-right: 10px;" :disabled="formData.audit !== '已审核'" @click="go">入库</Button>
        <ButtonGroup>
...
<!--        <Button style="margin-left: 10px;" :disabled="true">多账户付款</Button>-->
        <Button style="margin-left: 10px;" @click="createAdvance">生成预付单</Button>
      </Row>
...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'purchaseOrder') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '采购订单') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}采购订单的权力，请联系管理员`)
            }
          }
        })
      }
    ...
    loadParameter () {
      ...
      parameterPurchaseOrder({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `CGDD-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          supplierId: null,
          clerkId: null,
          departmentId: null,
    ...
    loadPurchaseOrder () {
      queryPurchaseOrder({
        ...
        this.isCanEdit = res.data.audit === '未审核' && res.data.close === '未关闭' && res.data.storage === '未入库'
      }).catch(err => {
        this.$Notice.error({
          title: '采购订单',
          desc: err.message
      ...
      this.$refs.formData.validate((valid) => {
        if (valid) {
          if (this.formData.list.length === 0) {
            ...
            updatePurchaseOrder(this.formData).then(res => {
    ...
    getInventory (warehouse, row, isUpdate) {
      let num = 0
      if (warehouse) {
        let iData = warehouse.inventoryList.find(f =>
          f.commodityId === row.commodityId &&
          (!f.auxiliary || (f.auxiliary === row.auxiliary)) &&
          (!row.isBatch || (f.batch && f.batch === row.batch)) &&
          (!row.isShelfLife || (f.generateDate && this.moment(f.generateDate).isSame(this.moment(row.generateDate), 'day'))) &&
          (!row.isShelfLife || (f.shelfLifeDays && f.shelfLifeDays === row.shelfLifeDays))
        )
        if (iData) num = iData.inventory
      }
      if (row.unit) {
    ...
    changePrice (index) {
      ...
      if (data.price !== null) {
        ...
        if (data.number !== null) {
          ...
          this.$set(this.formData.list[index], 'unStorage', NP.minus(NP.minus(data.number, data.inStorage || 0), data.reStorage || 0))
        }
      }
    ...
    batchFun (title, id, val) {
      ...
      if (title === '反审核') {
        if (this.formData.audit === '未审核') {
          ...
          return
        } else if (this.formData.storage !== '未入库') {
          this.$Message.error('该单据已有商品入库，不允许反审核')
          return
        }
      }
      this.$Modal.confirm({
        title: `${title}采购订单`,
        content: `确定${title}采购订单`,
        onOk: () => {
          batchPurchaseOrder({
            action: { audit: val },
            ids: [id]
          }).then(res => {
            this.$Message.success(`成功${title}采购订单`)
            this.loadPurchaseOrder()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    goNav (type) {
      let navList = this.purchaseOrder
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('purchaseOrder'))
      }
