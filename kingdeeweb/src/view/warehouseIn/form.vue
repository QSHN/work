        ...
        <Row type="flex">
          ...
          <FormItem label="业务类型" style="margin: 0 30px 0 0;">
            <Select v-model="formData.businessType" style="width:200px">
              <Option v-for="(item, index) in ['其他入库']" :value="item" :key="index">{{ item }}</Option>
            </Select>
          </FormItem>
        </Row>
      </Form>
...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'warehouseIn') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '其他入库') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}其他入库的权力，请联系管理员`)
            }
          }
        })
      }
    ...
    loadParameter () {
      ...
      parameterWarehouseIn({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `QTRK-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          supplierId: null,
          customerId: null,
          clerkId: null,
      ...
      this.$refs.formData.validate((valid) => {
        if (valid) {
          if (this.formData.list.length === 0) {
            ...
            updateWarehouseIn(this.formData).then(res => {
    ...
    batchFun (title, id, val) {
      ...
      this.$Modal.confirm({
        title: `${title}其他入库单`,
        content: `确定${title}其他入库单`,
        onOk: () => {
          batchWarehouseIn({
            action: { audit: val },
            ids: [id]
          }).then(res => {
            this.$Message.success(`成功${title}其他入库单`)
            this.loadWarehouseIn()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    goNav (type) {
      let navList = this.warehouseIn
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('warehouseIn'))
      }
