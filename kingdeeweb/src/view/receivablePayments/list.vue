...
import { queryReceivablePayments, batchReceivablePayments } from '../../api/receivablePayments'
...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      queryReceivablePayments(search).then(res => {
        let list = []
    ...
    batchFun (type, title, val = null) {
      ...
      selectList.forEach(id => {
        let fData = this.receivablePayments.find(f => f._id === id)
        if (title === '删除') {
          if (fData.audit === '已审核') {
            errorList.push({
              code: fData.code,
    ...
    pwFun () {
      ...
      selectList.forEach(id => {
        ...
        if (fData.main.payTotal >= fData.main.total) {
          error += `<P>${fData.code}预收金额已全部核销</P>`
        }
        if (customerId) {
