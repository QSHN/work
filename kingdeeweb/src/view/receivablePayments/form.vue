...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler () {
        if (this.$route.name !== 'receivablePayments') return
        if (this.accountSet) {
  ...
  methods: {
      ...
      if (this.myPermission && this.myPermission.length) {
        this.myPermission.forEach(f => {
          if (f.title === '预收款') {
            ...
            if (fData) {
              bool = fData.val
              if (!bool) this.$Message.error(`您没有${action}预收款的权力，请联系管理员`)
            }
          }
        })
      }
    ...
    loadParameter () {
      ...
      parameterReceivablePayments({
        ...
        this.formData = {
          accountSetId: this.accountSet._id,
          code: `YSKD-${time.split('-').join('')}-${total}`,
          dateTime: new Date(),
          customerId: null,
          clerkId: null,
          departmentId: null,
    ...
    loadSalesOrder () {
      querySalesOrder({
        ...
        res.data.list.forEach(f => {
          ...
          this.formData.orderId = f._id
          this.formData.main.list = [{
            advanceAccountId: f.advanceAccountId,
            advanceType: f.advanceType,
            advanceMoney: f.advanceMoney,
            transactionNo: null,
            des: null
          }]
        })
          ...
          if (this.formData.main.list.length === 0) {
            ...
            this.formData.main.list.forEach((v, i) => {
              if (!v.advanceAccountId || !v.advanceMoney || !v.advanceType) {
                error = `应收信息第${i + 1}条收款账户、收款方式和收款金额必填`
              }
            })
          }
          if (error) {
            ...
            if ((this.formData.main.payTotal || 0) > this.formData.main.total) {
              error = `收款金额总和小于核销总额，收款金额总和：${this.formData.main.total}，核销总额：${this.formData.main.payTotal}`
              this.$Message.error(error)
            } else {
              ...
              updateReceivablePayments(this.formData).then(res => {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          batchReceivablePayments({
            ...
            this.$Message.success(`成功${title}预收款单`)
            this.loadReceivablePayments()
          }).catch(err => {
            this.$Message.error(err.message)
          })
    ...
    goNav (type) {
      let navList = this.receivablePayments
      if (!navList.length) {
        navList = JSON.parse(localStorage.getItem('receivablePayments'))
      }
