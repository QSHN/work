<template>
  <div>
    ...
    <Table :width="getTableWidth"
      ...
      <template slot-scope="{ row }" slot="totalMoney">
        {{ row.totalMoney && formatMoney(row.totalMoney, '', 2) }}
      </template>
      ...
      <template slot-scope="{ row }" slot="customerClerkId">
        {{ clerkList.find(f => f._id === row.customerClerkId) && clerkList.find(f => f._id === row.customerClerkId).name }}
      </template>
      <template slot-scope="{ row }" slot="customerDepartmentId">
        {{ departmentList.find(f => f._id === row.customerDepartmentId) && departmentList.find(f => f._id === row.customerDepartmentId).name }}
      </template>
      <template slot-scope="{ row }" slot="supplierId">
        {{ selectList.find(f => f._id === row.supplierId) && selectList.find(f => f._id === row.supplierId).name }}
      </template>
      <template slot-scope="{ row }" slot="supplierClerkId">
        {{ clerkList.find(f => f._id === row.supplierClerkId) && clerkList.find(f => f._id === row.supplierClerkId).name }}
      </template>
      <template slot-scope="{ row }" slot="supplierDepartmentId">
        {{ departmentList.find(f => f._id === row.supplierDepartmentId) && departmentList.find(f => f._id === row.supplierDepartmentId).name }}
      </template>
...
import { queryReceivableVerification, batchReceivableVerification } from '../../api/receivableVerification'
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          title: '客户-业务员',
          width: 150,
          slot: 'customerClerkId',
          resizable: true
        },
        {
          title: '客户-部门',
          width: 150,
          slot: 'customerDepartmentId',
          resizable: true
        },
        {
          title: '供应商',
        ...
        },
        {
          title: '供应商-业务员',
          width: 150,
          slot: 'supplierClerkId',
          resizable: true
        },
        {
          title: '供应商-部门',
          width: 150,
          slot: 'supplierDepartmentId',
          resizable: true
        },
        {
          title: '单据备注',
  ...
  methods: {
    ...
    load () {
      ...
      queryReceivableVerification(search).then(res => {
        let list = []
        res.data.list.forEach(f => {
          list.push({
            ...
            customerClerkId: f.customerClerkId,
            customerDepartmentId: f.customerDepartmentId,
            supplierId: f.supplierId,
            supplierClerkId: f.supplierClerkId,
            supplierDepartmentId: f.supplierDepartmentId,
            totalMoney: f.main.total,
            audit: f.audit,
            des: f.des,
            creator: f.creator,
            auditor: f.auditor,
    ...
    batchFun (type, title, val = null) {
      ...
      selectList.forEach(id => {
        let fData = this.receivableVerification.find(f => f._id === id)
        if (title === '删除') {
          if (fData.audit === '已审核') {
            errorList.push({
              code: fData.code,
